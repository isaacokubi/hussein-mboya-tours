...
const NAV = {
  customer: [
    ...
    ["/tours", "Explore Tours", FileText],
    ["/profile", "Profile", UserRound],
    ["/wishlist", "Wishlist", Heart],
  ],
  guide: [
    ["/guide/dashboard", "Dashboard", LayoutDashboard],
    ["/guide/assigned-tours", "Assigned Tours", CalendarCheck],
    ["/profile", "Profile", UserRound],
  ],
  driver: [
        ...
        <div className="fixed inset-0 z-[60] lg:hidden">
          ...
          <aside className="relative h-full w-80 max-w-[86vw] bg-slate-950 p-5 text-white shadow-2xl">
            <div className="mb-8 flex items-center justify-between">
              <div><div className="text-lg font-bold">{displayTitle}</div><div className="text-xs text-slate-400 capitalize">{role} portal</div></div>
            ...
            <nav className="space-y-2">
              {items.map(([to, label, Icon]) => <Link key={to} to={to} onClick={() => setOpen(false)} className="flex items-center gap-3 rounded-xl px-4 py-3 font-medium hover:bg-white/10"><Icon size={19} />{label}</Link>)}
            </nav>
          </aside>
        </div>
      )}
