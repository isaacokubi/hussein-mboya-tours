...
test("subscription plans have explicit commercial feature entitlements", () => {
  ...
  assert.match(service, /billing/);
  assert.match(service, /etims/);
  assert.match(service, /ai/);
  assert.match(service, /developer_platform/);
});
...
test("tenant APIs enforce plan entitlements server-side", () => {
  ...
  assert.match(middleware, /PLAN_FEATURE_LOCKED/);
  assert.match(middleware, /getPlanFeaturesForTenant/);
  assert.match(routes, /router\.use\(enforcePlanFeature\)/);
});
...
test("SuperAdmin can configure plan feature entitlements", () => {
  ...
  const model = read("models/SystemSetting.js");
  assert.match(model, /subscriptionPlanFeatures/);
  assert.match(controller, /validatePlanFeatures/);
  assert.match(controller, /subscriptionPlanFeatures/);
  assert.match(routes, /billing\/features/);
});
...
test("tenant subscription response exposes the active plan feature set", () => {
  ...
  assert.match(controller, /getTenantPlanFeatures/);
  assert.match(controller, /features,/);
  assert.match(controller, /featureCatalog/);
});
