...
function textValue(modelName, field, i, def, tenantId) {
  ...
  if (f === "email") return `${modelName.toLowerCase()}-${t}-${i + 1}@test.example`;
  if (f.includes("phone") || f.includes("mobile")) return `07${String((10000000 + ((Number.parseInt(t.slice(-6), 36) || 1) + i) % 90000000)).slice(-8)}`;
  ...
  if (f.includes("url") || f.includes("link") || f.includes("website") || f.includes("video")) return `https://example.com/${modelName.toLowerCase()}/${t}/${i + 1}`;
  if (f.includes("image") || f === "avatar") return `https://example.com/${modelName.toLowerCase()}/${t}/${i + 1}.jpg`;
  if (f.includes("slug")) return `${modelName.toLowerCase()}-${t}-${i + 1}`;
  if (f.includes("code") || f.includes("reference") || f.includes("number")) return `${modelName.toUpperCase()}-${t}-${String(i + 1).padStart(4, "0")}`;
  if (f.includes("title") || f.includes("name")) return `${modelName} Test ${t} ${i + 1}`;
  if (f.includes("description") || f.includes("summary") || f.includes("notes") || f.includes("comment") || f.includes("reason")) return `Synthetic ${field} for ${modelName} tenant ${t} record ${i + 1}.`;
  if (f.includes("location") || f.includes("address")) return `${CITIES[i % CITIES.length]}, ${COUNTRIES[i % COUNTRIES.length]}`;
  return `${field}-${t}-${i + 1}`;
}
...
function numberValue(field, i, def, tenantId) {
  ...
  const tnum = Number.parseInt(token(tenantId).slice(-5), 36) || 1;
  const f = field.toLowerCase();
  let value = i + 1 + tnum;
  if (/(price|amount|fare|cost|revenue|balance|deposit|fee|commission)/.test(f)) value = 1500 + i * 500 + tnum;
...
function buildSubdoc(schema, modelName, i, pools, tenantId, depth = 0) {
  const out = {};
...
function buildValue(model, field, def, i, pools, tenantId, depth = 0) {
  ...
  if (def.instance === "Mixed") return { seeded: true, source: "allTenantsTestDataSeedV4", tenant: token(tenantId), index: i + 1 };
  return undefined;
}
...
function makeSchemaSafeCustomizations(model, doc, i, tenantId, pools) {
  ...
  if (name === "Tour") {
    doc.set("title", `Tenant ${t} Safari Tour ${i + 1}`);
    doc.set("slug", undefined);
    doc.set("published", true);
    doc.set("available", true);
    doc.set("status", "upcoming");
    doc.set("date", dateValue("date", i));
    doc.set("price", 5000 + i * 1000 + (Number.parseInt(t.slice(-4), 36) || 1));
  }
  if (name === "TourPackage") {
    if (model.schema.path("name")) doc.set("name", `Tenant ${t} Tour Package ${i + 1}`);
  ...
  if (name === "Booking") {
    ...
    if (model.schema.path("subtotal")) doc.set("subtotal", total);
    if (model.schema.path("depositAmount")) doc.set("depositAmount", Math.round(total / 2));
    if (model.schema.path("balanceAmount")) doc.set("balanceAmount", Math.round(total / 2));
    if (model.schema.path("paymentStatus")) doc.set("paymentStatus", "partial");
  ...
  if (name === "User") {
    ...
    if (model.schema.path("role")) doc.set("role", ROLES[i % ROLES.length]);
    if (model.schema.path("status")) doc.set("status", "active");
    if (model.schema.path("isVerified")) doc.set("isVerified", true);
    doc.set("email", `user-${t}-${i + 1}@example.test`);
    if (model.schema.path("phone")) doc.set("phone", `07${String((10000000 + i + (Number.parseInt(t.slice(-6), 36) || 1)) % 90000000).padStart(8, "0")}`);
  }
  return true;
}

function enforceUniqueIndexes(model, doc, i, tenantId) {
  const t = token(tenantId);
  for (const field of uniqueSingleFields(model)) {
    if (field === "slug" && ["Tour", "TourPackage"].includes(model.modelName)) continue;
    const current = doc.get(field);
    if (current === undefined || current === null || current === "") continue;
    if (typeof current === "string") doc.set(field, `${current}-${t}`.replace(/--+/g, "-"));
    else if (typeof current === "number") doc.set(field, current + (Number.parseInt(t.slice(-4), 36) || 1) + i * 10000);
  }
}
...
async function seedTenant(models, tenant) {
  ...
  const passes = [];
  for (let pass = 1; pass <= models.length + 4; pass += 1) {
    let progress = 0;
    for (const model of models) {
    ...
    const incomplete = models.some((model) => {
      if (isGlobal(model) || model.modelName === "Subscription") return false;
      const minimum = desiredFor(model, 0, pools);
...
async function main() {
  ...
  try {
    ...
    const complete = summaries.every((s) => tenantModels.every((m) => {
      const minimum = Number(s.counts[m.modelName] || 0);
      if (["Itinerary", "TourGallery", "Wishlist"].includes(m.modelName)) return minimum >= Math.min(TARGET, 10);
