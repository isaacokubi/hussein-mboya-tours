...
const repair = async () => {
  ...
  await runWithTenant({ role: "super_admin", bypass: true }, async () => {
    const destinations = await Destination.find({ slug: { $in: destinationSlugs } }).select("_id slug").lean();
    ...
    const tourSlugs = tourTitles.map(slugifyTitle);
    const tours = await Tour.find({ slug: { $in: tourSlugs } }).select("_id slug").lean();
    const tourUpdates = await Promise.all(
      tours.map((tour, index) => {
        const image = IMAGE_POOL[(index + 2) % IMAGE_POOL.length];
        const galleryImage = IMAGE_POOL[(index + 3) % IMAGE_POOL.length];
        return Tour.updateOne(
          { _id: tour._id },
          {
            $set: {
              featuredImage: { url: image, publicId: "" },
              gallery: [
                { url: image, publicId: "" },
    ...
    const destinationMatched = destinationUpdates.reduce((sum, result) => sum + result.matchedCount, 0);
    const tourMatched = tourUpdates.reduce((sum, result) => sum + result.matchedCount, 0);
    console.log(`Global Tours media repair complete: ${destinationMatched} destinations + ${tourMatched} tours updated.`);
