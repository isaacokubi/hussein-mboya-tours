...
const IMAGE_POOL = [
  ...
  image("photo-1549366021-9f761d450615"), image("photo-1510414842594-a61c69b5ae57"),
  image("photo-1507525428034-b723cf961d3e"), image("photo-1464822759023-fed622ff2c3b"),
  image("photo-1501785888041-af3ef285b470"), image("photo-1526772662000-3f88f10405ff"),
];
...
const TOUR_TEMPLATES = [
  ["Maasai Mara Safari Adventure", "Maasai Mara", "Safari", 4, 850], ["Amboseli Wildlife Escape", "Amboseli", "Safari", 3, 690],
  ["Tsavo Wildlife Explorer", "Tsavo National Park", "Safari", 4, 820], ["Naivasha Lakeside Escape", "Lake Naivasha", "Nature", 2, 380],
  ["Hell's Gate Cycling Adventure", "Lake Naivasha", "Adventure", 2, 450], ["Watamu Marine Discovery", "Watamu", "Beach", 3, 610],
  ["Diani Beach Retreat", "Diani Beach", "Beach", 4, 720], ["Mount Kenya Highland Adventure", "Mount Kenya", "Mountain", 4, 950],
  ["Samburu Wildlife Discovery", "Samburu National Reserve", "Safari", 3, 780], ["Kenya Family Safari", "Maasai Mara", "Family", 5, 1180],
];

const PERMISSIONS = [
  "manage_users", "manage_tours", "manage_destinations", "manage_bookings", "manage_payments", "view_reports",
  "manage_gallery", "manage_staff", "manage_vehicles", "manage_settings", "view_assigned_tours", "view_tour_guests",
  "update_tour_status", "submit_tour_report", "create_bookings", "manage_customer_bookings", "view_tours",
  "view_destinations", "view_commissions", "view_wallet",
];
const ROLE_DEFS = [["super_admin", "Super Admin", 100], ["admin", "Admin", 90], ["tour_manager", "Tour Manager", 70], ["agent", "Travel Agent", 50], ["tour_guide", "Tour Guide", 40], ["driver", "Driver", 30], ["customer", "Customer", 10]];
...
const run = async () => {
  ...
  for (const collection of oldCollections) await db.collection(collection.name).drop().catch(() => {});

  const now = new Date();
  const hashedPassword = await bcrypt.hash(PASSWORD, 12);
  const permissionDocs = PERMISSIONS.map((name) => ({ _id: oid(), name, label: name.replace(/_/g, " "), module: name.split(/[._]/)[0], category: "other", description: `Permission to ${name.replace(/_/g, " ")}`, isActive: true, createdAt: now, updatedAt: now }));
  await db.collection("permissions").insertMany(permissionDocs);
  const roleDocs = ROLE_DEFS.map(([name, displayName, level]) => ({ _id: oid(), name, displayName, level, isSystem: true, status: "active", permissions: permissionDocs.map((p) => p._id), createdAt: now, updatedAt: now }));
  ...
  const organizations = TENANTS.map((tenant) => ({
    _id: oid(), name: tenant.name, slug: tenant.slug, legalName: tenant.name, supportEmail: tenant.email,
    supportPhone: tenant.phone, address: tenant.address, country: "Kenya", timezone: "Africa/Nairobi", currency: "KES", status: "active",
    ...
    settings: { companyName: tenant.name, city: tenant.address.split(",")[0], country: "Kenya" }, createdAt: now, updatedAt: now,
  }));
  ...
  for (let t = 0; t < organizations.length; t += 1) {
    ...
    await db.collection("customers").insertMany(tenantCustomers);
    await db.collection("vehicles").insertMany(tenantVehicles);

    const destinationDocs = DESTINATIONS.map(([name, slug, region, description], index) => ({ _id: oid(), tenantId: tenant._id, name, slug, country: "Kenya", region, shortDescription: description, description, images: [{ url: IMAGE_POOL[(index + t) % IMAGE_POOL.length] }], featuredImage: IMAGE_POOL[(index + t) % IMAGE_POOL.length], attractions: ["Scenic Landscapes", "Wildlife", "Local Culture"], activities: ["Guided Tours", "Photography", "Sightseeing"], languages: ["English", "Swahili"], currency: "KES", timezone: "Africa/Nairobi", bestSeason: "All Year", featured: index < 5, popular: true, status: "active", active: true, isDeleted: false, createdAt: now, updatedAt: now }));
    ...
    const tourDocs = TOUR_TEMPLATES.map(([title, destinationName, category, days, price], index) => {
      const destination = destinationByName.get(destinationName); const startDate = new Date(now); startDate.setDate(startDate.getDate() + 10 + index * 4 + t * 2); const tourImage = IMAGE_POOL[(index + 2 + t) % IMAGE_POOL.length];
      return { _id: oid(), tenantId: tenant._id, title, slug: `${slugify(title)}-${suffix}`, description: `${days}-day ${category.toLowerCase()} experience in ${destinationName}, Kenya, professionally planned by ${company.name}.`, shortDescription: `${days}-day ${category.toLowerCase()} experience in ${destinationName}.`, tags: [company.name, category, destinationName, "Kenya"], category, destination: destination._id, country: "Kenya", location: destination.region, meetingPoint: "Nairobi CBD", duration: `${days} Days`, durationDetails: { days, nights: Math.max(0, days - 1) }, date: startDate, startDate, capacity: 20, price, agentPrice: Math.max(0, price - 50), discount: index % 4 === 0 ? 5 : 0, featuredImage: { url: tourImage }, gallery: [{ url: tourImage }, { url: IMAGE_POOL[(index + 3) % IMAGE_POOL.length] }], highlights: ["Professional local guide", "Comfortable transport", "Authentic Kenyan experience"], inclusions: ["Professional guide", "Transport", "Accommodation", "Selected activities"], exclusions: ["International flights", "Travel insurance", "Personal expenses"], languages: ["English", "Swahili"], difficulty: category === "Mountain" ? "moderate" : "easy", itinerary: Array.from({ length: days }, (_, dayIndex) => ({ day: dayIndex + 1, title: dayIndex === 0 ? "Arrival and Orientation" : dayIndex === days - 1 ? "Final Experience and Departure" : "Explore and Experience", description: `Discover ${destinationName} with guided activities and scenic stops.`, activities: ["Sightseeing", "Photography", "Guided exploration"] })), availabilitySettings: { totalSlots: 20, bookedSlots: 0, waitlistEnabled: true }, bookingDeadline: 1, instantBooking: true, cancellationPolicy: "Free cancellation subject to company policy.", featured: index < 6, status: "upcoming", published: true, available: true, isDeleted: false, averageRating: 4.5, totalReviews: 12 + index, popularity: 80 - index, createdBy: admin._id, createdAt: now, updatedAt: now };
    });
    await db.collection("tours").insertMany(tourDocs);
    await db.collection("tourcategories").insertMany([["Wildlife Safari", "wildlife-safari", "Wildlife and national park experiences."], ["Beach Holidays", "beach-holidays", "Indian Ocean beach and marine experiences."], ["Mountain Adventures", "mountain-adventures", "Mountain hiking and highland adventures."], ["Family Tours", "family-tours", "Comfortable experiences designed for families."]].map(([name, slug, description], index) => ({ _id: oid(), tenantId: tenant._id, name, slug: `${slug}-${suffix}`, description, icon: ["Binoculars", "Beach", "Mountain", "Users"][index], active: true, createdAt: now, updatedAt: now })));
    await db.collection("galleries").insertMany(Array.from({ length: 8 }, (_, index) => ({ _id: oid(), tenantId: tenant._id, title: `${company.name} Experience ${index + 1}`, image: { url: IMAGE_POOL[(index + t) % IMAGE_POOL.length], publicId: "" }, category: index < 4 ? "Safari" : "Beach", featured: true, active: true, createdAt: now, updatedAt: now })));
    await db.collection("heroslides").insertMany([["Discover Kenya Beyond the Ordinary", "Premium safaris, coastal escapes and unforgettable East African adventures."], ["Wild Kenya. Beautifully Yours.", "Travel with local expertise across wildlife reserves, mountains, lakes and the coast."], ["From Safari Trails to the Indian Ocean", "Build your perfect Kenyan journey with experiences designed around you."]].map(([title, subtitle], index) => ({ _id: oid(), tenantId: tenant._id, title, subtitle, badge: company.name.toUpperCase(), image: { url: IMAGE_POOL[(index + t) % IMAGE_POOL.length], publicId: "" }, buttonOne: { text: "Explore Tours", link: "/tours" }, buttonTwo: { text: "Contact Us", link: "/contact" }, active: true, order: index + 1, createdAt: now, updatedAt: now })));

    const bookingSpecs = [
      { customer: tenantCustomers[0], tour: tourDocs[0], status: "confirmed", paymentStatus: "paid", guests: 2 },
      { customer: tenantCustomers[1], tour: tourDocs[3], status: "pending", paymentStatus: "partial", guests: 1 },
      { customer: tenantCustomers[0], tour: tourDocs[6], status: "completed", paymentStatus: "paid", guests: 2 },
    ...
    await db.collection("bookings").bulkWrite(bookingDocs.map((booking, index) => ({ updateOne: { filter: { _id: booking._id }, update: { $set: { payments: [paymentDocs[index]._id] } } } })));
    await db.collection("reviews").insertMany(tourDocs.slice(0, 4).map((tour, index) => ({ _id: oid(), tenantId: tenant._id, user: tenantCustomers[index % 2].user, customer: tenantCustomers[index % 2]._id, tour: tour._id, rating: 5, title: "Excellent experience", comment: `A memorable ${tour.title} with professional service.`, status: "approved", approved: true, createdAt: now, updatedAt: now })));
