...
export async function createTenant(req, res, next) {
  try {
    ...
    if (admin && Object.values(admin).some(Boolean) && !(admin.name && admin.email && admin.phone && admin.password)) {
      return res.status(400).json({ success: false, message: "Provide name, email, phone and password for the first company administrator." });
    }
    ...
    const existing = await Organization.findOne({ slug: tenantSlug }).lean();
    if (existing) return res.status(409).json({ success: false, message: "That company slug is already in use." });

    const organization = await Organization.create({
      name: name.trim(), slug: tenantSlug, country, timezone, currency,
    ...
    if (admin?.name && admin?.email && admin?.phone && admin?.password) {
      adminUser = await runWithTenant({ tenantId: organization._id, tenant: organization, bypass: false }, async () => {
        ...
        if (!role) role = await Role.create({ name: "admin", displayName: "Administrator", description: "Tenant administrator", isSystem: true, level: 100 });
...
export async function getTenant(req, res, next) {
  try {
    const { id } = req.params;
    if (!mongoose.Types.ObjectId.isValid(id)) return res.status(400).json({ success: false, message: "Invalid tenant ID." });
