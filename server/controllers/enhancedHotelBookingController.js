...
export async function createEnhancedHotelBooking(req, res, next) {
  try {
    ...
    const overlap = await HotelBooking.aggregate([
      { $match: { tenantId, roomType: room._id, status: { $nin: ["cancelled", "no_show"] }, checkIn: { $lt: checkOut }, checkOut: { $gt: checkIn } } },
      { $group: { _id: null, rooms: { $sum: "$rooms" } } },
    ]);
    const bookedRooms = Number(overlap[0]?.rooms || 0);
    const available = Math.max(Number(room.totalRooms || 0) - bookedRooms, 0);
    ...
    const booking = await HotelBooking.create({
      ...
      mealPlan: req.body.mealPlan || "room_only",
      ...
      currency: hotel.currency || room.currency || "KES",
