...
async function resolveCustomer(req, body) {
  ...
  const nameParts = clean(`${req.user.firstName || ""} ${req.user.lastName || ""} ${req.user.name || ""}`)
    .trim()
    .split(/\s+/)
    .filter(Boolean);
  ...
  return Customer.create({
    ...
    firstName,
    lastName,
    email: clean(body.email || req.user.email),
    phone,
...
export async function createEnhancedHotelBooking(req, res, next) {
  try {
    ...
    if (!tenantId || !hotelId || !roomTypeId) {
      return res.status(400).json({
        success: false,
        message: "Hotel, room type and tenant context are required.",
      });
    }

    const [hotel, room] = await Promise.all([
      Hotel.findOne({ _id: hotelId, tenantId, status: "active" }).lean(),
      HotelRoomType.findOne({
        _id: roomTypeId,
        tenantId,
        hotel: hotelId,
        status: "active",
    ...
    if (!hotel || !room) {
      return res.status(404).json({ success: false, message: "Hotel or room type not found." });
    }
    ...
    if (
      ...
      return res.status(400).json({
        success: false,
        message: "Valid check-in, check-out, rooms and guests are required.",
      });
    }
    ...
    if (adults > rooms * Number(room.maxAdults || 1) || children > rooms * Number(room.maxChildren || 0)) {
      return res.status(400).json({
        success: false,
        message: "Guest count exceeds the selected room capacity.",
      });
    }
    ...
    const [overlap, blockAgg, rates] = await Promise.all([
      ...
      HospitalityRatePlan.find({
        ...
        $and: [{ $or: [{ validTo: null }, { validTo: { $gte: checkOut } }] }],
      })
        .sort({ nightlyRate: 1 })
        .lean(),
    ]);
    ...
    const rate =
      rates.find(
        (item) =>
          nights >= Number(item.minNights || 1) &&
          (!item.maxNights || nights <= Number(item.maxNights))
      ) || null;

    if (rate?.stopSell) {
      return res.status(409).json({
        success: false,
        message: "This room is currently stop-sold for the selected stay.",
      });
    }

    const availableRooms = Math.max(
      Number(room.totalRooms || 0) - bookedRooms - blockedRooms,
      0
    );
    ...
    if (rooms > availableRooms) {
      return res.status(409).json({
        success: false,
        message: `Only ${availableRooms} room(s) are available for those dates.`,
      });
    }
    ...
    if (req.user?.role === "customer" && !customer) {
      return res.status(400).json({
        success: false,
        message: "A customer phone number is required to complete the booking.",
      });
    }
    ...
    const booking = await HotelBooking.create({
      ...
      checkIn,
      checkOut,
      estimatedArrivalTime: clean(req.body.estimatedArrivalTime),
      rooms,
      adults,
      children,
      guests: Array.isArray(req.body.guests) ? req.body.guests : [],
      mealPlan: req.body.mealPlan || rate?.mealPlan || "room_only",
      bedPreference: clean(req.body.bedPreference),
      dietaryRequirements: clean(req.body.dietaryRequirements),
      accessibilityNeeds: clean(req.body.accessibilityNeeds),
      airportTransferRequired: Boolean(req.body.airportTransferRequired),
      ...
      fees,
      totalAmount,
      currency: rate?.currency || hotel.currency || room.currency || "KES",
