...
export const createWithholdingTax = async (req, res, next) => {
  try {
    ...
    const data = await WithholdingTax.create({ ...req.body, tenantId, baseAmount, rate, taxAmount, taxPeriod: String(req.body?.taxPeriod || currentPeriod()), createdBy: actor(req), updatedBy: actor(req) });
