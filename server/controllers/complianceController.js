...
import { enqueueInvoiceForEtims } from "../services/etimsService.js";
...
export async function listComplianceRecords(req, res, next) {
  try { requireTenantId(); const data = await ComplianceRecord.find(mergeTenantFilter(req, req.query?.type ? { type: req.query.type } : {})).populate("owner", "name email").sort({ expiryDate: 1, type: 1 }).lean(); return res.json({ success: true, data }); } catch (error) { next(error); }
}
...
export async function getComplianceSummary(req, res, next) {
  try { requireTenantId(); const records = await ComplianceRecord.find(mergeTenantFilter(req, {})).lean(); const now = Date.now(); const expiringSoon = records.filter((r) => r.expiryDate && new Date(r.expiryDate).getTime() <= now + 30 * 86400000 && new Date(r.expiryDate).getTime() >= now).length; const expired = records.filter((r) => r.expiryDate && new Date(r.expiryDate).getTime() < now).length; const actionRequired = records.filter((r) => ["action_required", "expired"].includes(r.status)).length; return res.json({ success: true, data: { total: records.length, expiringSoon, expired, actionRequired, records } }); } catch (error) { next(error); }
}

export async function queueEtimsInvoice(req, res, next) {
  try {
    const tenantId = requireTenantId();
    if (!mongoose.isValidObjectId(req.params.id)) return res.status(400).json({ success: false, message: "Invalid invoice ID." });
    const invoice = await Invoice.findOne(mergeTenantFilter(req, { _id: req.params.id, isDeleted: { $ne: true } })).select("_id tenantId etimsStatus invoiceNumber").lean();
    if (!invoice) return res.status(404).json({ success: false, message: "Invoice not found." });
    const job = await enqueueInvoiceForEtims(invoice._id, tenantId);
    return res.status(202).json({ success: true, message: "Invoice queued for eTIMS adapter submission.", data: { invoice, jobId: job._id, jobStatus: job.status } });
  } catch (error) { next(error); }
}
