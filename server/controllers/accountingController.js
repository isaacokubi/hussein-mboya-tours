...
async function ensureDefaults(tenantId) {
  for (const [code, name, type, subtype] of DEFAULT_ACCOUNTS) await ChartOfAccount.updateOne({ tenantId, code }, { $setOnInsert: { tenantId, code, name, type, subtype, currency: "KES", system: true } }, { upsert: true });
}
...
export const listJournalEntries = async (req, res, next) => {
  try {
    const filter = tenantFilter(req); if (req.query.status) filter.status = req.query.status;
    if (req.query.from || req.query.to) { filter.entryDate = {}; if (req.query.from) filter.entryDate.$gte = new Date(req.query.from); if (req.query.to) { const to = new Date(req.query.to); to.setHours(23, 59, 59, 999); filter.entryDate.$lte = to; } }
    return ok(res, await JournalEntry.find(filter).populate("lines.account", "code name type").sort({ entryDate: -1, createdAt: -1 }).limit(Math.min(Number(req.query.limit) || 200, 500)).lean());
  } catch (e) { return next(e); }
};

export const createJournalEntry = async (req, res, next) => {
  try {
    await assertTenantAccounts(req, req.body?.lines);
...
export const postJournalEntry = async (req, res, next) => {
  try {
    ...
    if (entry.status === "posted") return ok(res, entry); if (entry.status === "void") return res.status(409).json({ success: false, message: "Void entries cannot be posted." });
    await assertTenantAccounts(req, entry.lines);
    const debit = entry.lines.reduce((s, l) => s + Number(l.debit || 0), 0); const credit = entry.lines.reduce((s, l) => s + Number(l.credit || 0), 0);
