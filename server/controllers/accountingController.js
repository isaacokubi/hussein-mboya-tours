...
async function ensureDefaults(tenantId) {
  ...
  const legacy = await ChartOfAccount.findOne({ ...tenantFilter({ tenantId }), code: "6000", active: { $ne: false } }).select("_id").lean();
  if (legacy) {
    const used = await JournalEntry.exists({ ...tenantFilter({ tenantId }), "lines.account": legacy._id });
    if (!used) await ChartOfAccount.updateOne({ ...tenantFilter({ tenantId }), _id: legacy._id }, { $set: { active: false, system: true } });
  }
}
...
export const getLedgerSummary = async (req, res, next) => {
  try {
    await ensureDefaults(req.tenantId); const accounts = await ChartOfAccount.find({ ...tenantFilter(req), active: { $ne: false } }).sort({ code: 1 }).lean(); const entries = await JournalEntry.find({ ...tenantFilter(req), status: "posted", $nor: [{ description: DEMO_MARKER }, { reference: DEMO_MARKER }] }).select("lines").lean();
