...
async function ensureDefaults(tenantId) {
  ...
  if (legacy) { const used = await JournalEntry.exists({ ...filter, "lines.account": legacy._id }); if (!used) await ChartOfAccount.updateOne({ ...filter, _id: legacy._id }, { $set: { active: false, system: true } }); }
}
...
export const getLedgerSummary = async (req, res, next) => {
  try {
    await ensureDefaults(req.tenantId); const accounts = await ChartOfAccount.find({ ...tenantFilter(req), active: { $ne: false } }).sort({ code: 1 }).lean(); const entries = await JournalEntry.find({ ...tenantFilter(req), status: "posted" }).select("lines").lean();
