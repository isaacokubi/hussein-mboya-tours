...
export const getCustomers = async (req, res, next) => {
  try {
    ...
    const [customers, total] = await Promise.all([
      User.find(filter).select("name email phone role roleId legacyRole status createdAt").populate("roleId", "name displayName").sort({ createdAt: -1 }).skip(skip).limit(pageSize).lean(),
    ...
    for (const booking of bookingStats) {
      ...
      if (!ownerId) continue;
      if (!statsMap[ownerId]) statsMap[ownerId] = { totalBookings: 0, totalSpent: 0 };
    ...
    const data = customers.map((customer) => {
      ...
      const legacy = customerRecords.find((record) => record.user?.toString() === customer._id.toString());
      return { ...customer, customerType: legacy?.customerType || "individual", isActive: customer.status === "active", totalBookings: stats.totalBookings, totalSpent: stats.totalSpent };
    });
