...
export const login = async (req, res, next) => {
  try {
    ...
    const token = generateToken({ _id: user._id, role: effectiveRole, roleId: user.roleId, email: user.email, permissions, tenantId: user.tenantId });
    ...
    return res.status(200).json({ success: true, token, user: publicUser(user, permissions), tenant: req.tenant || (user.tenantId ? await Organization.findById(user.tenantId).lean() : null) });
  } catch (error) {
...
export const register = async (req, res, next) => {
  try {
    ...
    const token = generateToken({ _id: user._id, role: "customer", roleId: user.roleId, email: user.email, permissions: [], tenantId: user.tenantId });
    return res.status(201).json({ success: true, token, user: publicUser(user, []), tenant: req.tenant || null });
  } catch (error) {
