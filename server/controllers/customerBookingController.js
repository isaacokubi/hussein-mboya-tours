...
export const createCustomerBooking = async (req, res, next) => {
  ...
  try {
    ...
    if (!tour || !travelDate) {
      return res.status(400).json({ success: false, message: "Tour and travel date are required." });
    }
    ...
    if (!capacityAvailable) {
      return res.status(409).json({ success: false, message: "Not enough available tour slots for this booking." });
    }

    const amounts = calculateBookingAmounts(tourData, guests);
    const totalAmount = Number(amounts.totalAmount || 0);
    if (!Number.isFinite(totalAmount) || totalAmount <= 0) {
      return res.status(409).json({ success: false, message: "This tour does not have a valid price." });
    }
    ...
    try {
      const booking = await Booking.create({
        ...
        specialRequests: Array.isArray(specialRequests)
          ? specialRequests.map(String).map((x) => x.trim()).filter(Boolean)
          : [],
      ...
      await releaseSlots(tourData._id, guests).catch((error) =>
        console.error("BOOKING CAPACITY ROLLBACK ERROR:", error)
      );
      throw createError;
    }
  } catch (error) {
