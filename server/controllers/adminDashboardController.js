...
export const getDashboard = async (req, res) => {
  ...
  try {
    ...
    const [users, tours, bookings, destinations, customers, adminsCount, guidesCount, vehiclesCount, agentsCount, guideUsersCount, agentUsersCount, driversCount, staffCount] = await Promise.all([
      ...
      Staff.countDocuments({ $or: [{ position: { $in: ["guide", "tour_guide", "tourguide"] } }, { role: { $in: ["guide", "tour_guide", "tourguide"] } }], isDeleted: NON_DELETED, isActive: { $ne: false }, status: { $ne: "inactive" } }),
      Vehicle.countDocuments({ isDeleted: NON_DELETED, isActive: { $ne: false }, status: { $nin: ["out_of_service", "retired", "inactive"] } }),
      User.countDocuments({ role: "agent" }),
      User.countDocuments({ role: { $in: ["guide", "tour_guide", "tourguide"] } }),
      Agent.countDocuments({ status: { $ne: "inactive" } }),
      Staff.countDocuments({ $or: [{ position: { $in: ["driver", "chauffeur"] } }, { role: { $in: ["driver", "chauffeur"] } }], isDeleted: NON_DELETED, isActive: { $ne: false }, status: { $ne: "inactive" } }),
      Staff.countDocuments({ isDeleted: NON_DELETED, isActive: { $ne: false }, status: { $ne: "inactive" } }),
    ]);
    ...
    const [paymentStats, revenueResult, monthlyRevenue, bookingStatus, popularTours, recentBookings, agentPerformance, guidePerformance, notifications] = await Promise.all([
      Payment.aggregate([{ $group: { _id: "$status", count: { $sum: 1 }, amount: { $sum: { $max: [0, { $subtract: [{ $ifNull: ["$amount", 0] }, { $ifNull: ["$refundedAmount", 0] }] }] } } } }]),
      Payment.aggregate([{ $match: { status: ACTIVE_PAYMENT_STATUS } }, { $group: { _id: null, total: { $sum: { $max: [0, { $subtract: [{ $ifNull: ["$amount", 0] }, { $ifNull: ["$refundedAmount", 0] }] }] } } } }]),
      Payment.aggregate([{ $match: { status: ACTIVE_PAYMENT_STATUS } }, { $group: { _id: { year: { $year: { $ifNull: ["$paidAt", "$createdAt"] } }, month: { $month: { $ifNull: ["$paidAt", "$createdAt"] } } }, amount: { $sum: { $max: [0, { $subtract: [{ $ifNull: ["$amount", 0] }, { $ifNull: ["$refundedAmount", 0] }] }] } } } }, { $sort: { "_id.year": 1, "_id.month": 1 } }]),
      Booking.aggregate([{ $match: { isDeleted: NON_DELETED } }, { $group: { _id: "$status", count: { $sum: 1 } } }, { $sort: { _id: 1 } }]),
      Booking.aggregate([
        { $match: { isDeleted: NON_DELETED, status: { $nin: ["cancelled", "refunded"] }, tour: { $ne: null } } },
        { $group: {
          _id: "$tour",
          totalBookings: { $sum: 1 },
          paidBookings: {
    ...
    const availableVehicles = await Vehicle.countDocuments({ isDeleted: NON_DELETED, isActive: true, status: "available" });
