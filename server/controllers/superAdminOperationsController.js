...
export const createDatabaseBackup = async (req, res) => { try { if (!mongoose.connection.db) return res.status(503).json({ success: false, message: "Database connection unavailable" }); const db = mongoose.connection.db; if (!fs.existsSync(BACKUP_DIR)) fs.mkdirSync(BACKUP_DIR, { recursive: true }); const filename = `database-backup-${Date.now()}.json`; const filepath = path.join(BACKUP_DIR, filename); const backupData = { createdAt: new Date(), environment: process.env.NODE_ENV || "production", database: mongoose.connection.name || "unknown", createdBy: req.user?.email || req.user?._id || "system" }; const collections = await db.listCollections().toArray(); const collectionNames = collections.map((collection) => collection.name); for (const collection of collections) backupData[collection.name] = await db.collection(collection.name).find(tenantFilter(req)).toArray(); fs.writeFileSync(filepath, JSON.stringify(backupData, null, 2)); await DatabaseBackup.create({ file: filename, size: `${(fs.statSync(filepath).size / 1024 / 1024).toFixed(2)} MB`, collections: collectionNames, databaseName: mongoose.connection.name || "unknown", environment: process.env.NODE_ENV || "production", createdBy: req.user?.email || req.user?._id || "system" }); await createAuditLog({ user: req.user?._id, action: "create", resource: "Database", description: `Database backup created: ${filename}`, status: "success", severity: "low" }); res.json({ success: true, message: "Database backup created successfully", file: filename }); } catch (error) { console.error("BACKUP ERROR DETAILS:", error.message, error.stack); res.status(500).json({ success: false, message: error.message }); } };
export const listDatabaseBackups = async (req, res) => { try { const backups = await DatabaseBackup.find(tenantFilter(req)).sort({ createdAt: -1 }).limit(10).lean(); res.json({ success: true, backups }); } catch (error) { console.error("LIST BACKUPS ERROR", error); res.status(500).json({ success: false, message: error.message }); } };
export const downloadDatabaseBackup = async (req, res) => { try { const backup = await DatabaseBackup.findOne(mergeTenantFilter(req, { _id: req.params.id })); if (!backup) return res.status(404).json({ success: false, message: "Backup not found" }); const filepath = path.join(BACKUP_DIR, backup.file); if (!fs.existsSync(filepath)) return res.status(404).json({ success: false, message: "Backup file missing" }); res.download(filepath); } catch (error) { await createAuditLog({ user: req.user?._id, action: "error", resource: "Database", description: error.message, status: "failed", severity: "high" }); res.status(500).json({ success: false, message: error.message }); } };
export const deleteDatabaseBackup = async (req, res) => { try { const backup = await DatabaseBackup.findOne(mergeTenantFilter(req, { _id: req.params.id })); if (!backup) return res.status(404).json({ success: false, message: "Backup not found" }); const filepath = path.join(BACKUP_DIR, backup.file); if (fs.existsSync(filepath)) fs.rmSync(filepath); await DatabaseBackup.findOneAndDelete(mergeTenantFilter(req, { _id: req.params.id })); res.json({ success: true, message: "Backup deleted successfully" }); } catch (error) { res.status(500).json({ success: false, message: error.message }); } };
