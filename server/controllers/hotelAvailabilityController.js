    ...
    const result = await Promise.all(rooms.map(async (room) => {
      const [booked, blocks, rates] = await Promise.all([
        ...
        HospitalityRatePlan.find({
          ...
          $and: [{ $or: [{ validTo: null }, { validTo: { $gte: end } }] }],
        }).sort({ nightlyRate: 1 }).lean(),
      ]);
      ...
      const stopSell = Boolean(rate?.stopSell);
      const available = stopSell ? 0 : availableBase;
      const nightlyRate = rate?.nightlyRate ?? room.nightlyRate;
