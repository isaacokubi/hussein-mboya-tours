...
export const getOperationsOverview = async (req, res, next) => {
  try {
    ...
    const [rawBookings, vehicles, staff, rawSchedule, suppliers, purchaseOrders, tourCosts, payables, corporateAccounts] = await Promise.all([
      ...
      Booking.find({ ...bookingFilter, status: { $in: ["confirmed", "assigned", "ongoing", "pending"] }, travelDate: { $gte: new Date() } }).populate("tour", "title").sort({ travelDate: 1 }).limit(50).lean(),
    ...
    res.json({ success: true, data: { bookings, schedule, vehicles, guides, drivers, procurement: { activeSuppliers: suppliers, purchaseOrderCount: purchaseOrders.length, purchaseOrderValue: procurementValue, totalTourCost, outstandingPayables, corporateExposure }, stats: { todayTrips: todayBookings.length, upcomingTrips: schedule.length, confirmedBookings: bookings.filter((b) => String(b.status).toLowerCase() === "confirmed").length, resourceCoverage: bookings.length ? Math.round((assigned / bookings.length) * 100) : 0, availableVehicles: vehicles.filter((v) => ["available", "active", "ready"].includes(String(v.status || "").toLowerCase()) || ["available", "active", "ready"].includes(String(v.availability || "").toLowerCase())).length, availableGuides: guides.filter((v) => ["available", "active", "ready"].includes(String(v.status || "").toLowerCase()) || ["available", "active", "ready"].includes(String(v.availability || "").toLowerCase())).length, availableDrivers: drivers.filter((v) => ["available", "active", "ready"].includes(String(v.status || "").toLowerCase()) || ["available", "active", "ready"].includes(String(v.availability || "").toLowerCase())).length } } });
  } catch (e) { next(e); }
};
