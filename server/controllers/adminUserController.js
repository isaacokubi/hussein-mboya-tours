...
export const getUsers = async (req, res, next) => {
  ...
  try {
    ...
    const [users, total] = await Promise.all([
      User.find(tenantQuery).select("-password").populate("roleId", "name displayName permissions").sort({ createdAt: -1 }).skip(skip).limit(limit).lean(),
...
export const createStaffAccount = async (req, res, next) => {
  let createdUser = null, createdStaff = null, createdAgent = null;
  try {
    ...
    createdUser = await createWithTenantIndexRepair(User, { name: name.trim(), email: normalizedEmail, phone: String(phone).trim(), password, role: canonicalRole, legacyRole: canonicalRole, roleId: roleDoc._id, tenantId, status: "active", isVerified: true });
    ...
    const safeUser = await User.findOne(mergeTenantFilter(req, { _id: createdUser._id })).select("-password").populate("roleId", "name displayName permissions").lean();
    ...
    if (createdUser?._id) { try { await Promise.allSettled([createdStaff?._id ? Staff.deleteOne({ _id: createdStaff._id, tenantId: createdUser.tenantId }) : Promise.resolve(), createdAgent?._id ? Agent.deleteOne({ _id: createdAgent._id, tenantId: createdUser.tenantId }) : Promise.resolve(), User.deleteOne({ _id: createdUser._id, tenantId: createdUser.tenantId })]); } catch {} }
...
export const updateUserStatus = async (req, res, next) => {
  ...
  try {
    ...
    const user = await User.findOneAndUpdate(mergeTenantFilter(req, { _id: id }), { $set: { status } }, { new: true, runValidators: true }).select("-password").lean();
...
export const deleteUser = async (req, res, next) => {
  ...
  try {
    ...
    const user = await User.findOne(mergeTenantFilter(req, { _id: id })).select("_id role tenantId");
    if (!user) return res.status(404).json({ success: false, message: "User not found" });
    ...
    const tenantFilter = mergeTenantFilter(req, {});
    await Promise.all([Staff.deleteMany({ ...tenantFilter, user: user._id }), Agent.deleteMany({ ...tenantFilter, user: user._id }), User.deleteOne({ ...tenantFilter, _id: user._id })]);
