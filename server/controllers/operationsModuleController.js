...
const LIVE_ONLY = { $nor: [{ notes: /synthetic|DEMO[-_]/i }] };
...
export const listSupplierPayables=async(req,res,next)=>{try{return ok(res,await SupplierPayable.find(tenantFilter(req,LIVE_ONLY)).populate("supplier","legalName").sort({dueDate:1,createdAt:-1}).lean());}catch(e){return fail(next,e);}};
...
export const listCorporateAccounts=async(req,res,next)=>{try{return ok(res,await CorporateAccount.find(tenantFilter(req,LIVE_ONLY)).sort({companyName:1}).lean());}catch(e){return fail(next,e);}};
