  ...
  try {
    ...
    const [totalBookings, revenueResult, bookingStatus, popularTours, monthlyRevenue, totalCustomers, totalTours, completedTours] = await Promise.all([
      ...
      Payment.aggregate([
        ...
        { $unwind: "$bookingDoc" },
        { $match: mergeTenantFilter(req, { "bookingDoc.isDeleted": { $ne: true } }) },
        { $group: { _id: "$bookingDoc.tour", bookings: { $addToSet: "$bookingDoc._id" }, revenue: { $sum: { $max: [0, { $subtract: [{ $ifNull: ["$amount", 0] }, { $ifNull: ["$refundedAmount", 0] }] }] } } } },
        ...
        { $unwind: "$tour" },
        { $match: mergeTenantFilter(req, { "tour.isDeleted": { $ne: true } }) },
