...
export const createCustomTourRequest=async(req,res,next)=>{
  ...
  try {
    ...
    const item=await CustomTourRequest.create({
      customer:isAuthenticated?req.user._id:null,user:isAuthenticated?req.user._id:null,
      guestContact:{name,email,phone},destination:String(destination).trim(),durationDays:Number(durationDays),people:Number(people),
      startDate:startDate?new Date(startDate):null,budget:Number(budget||0),requirements:String(requirements||"").trim(),
      pickupLocation:String(pickupLocation||"").trim(),pickupDate:pickupDate?new Date(pickupDate):null,pickupTime:String(pickupTime||"").trim(),
      adults:Number(adults||people||1),children:Number(children||0),accommodationPreference:String(accommodationPreference||"").trim(),
      mealPreference:String(mealPreference||"").trim(),transportPreference:String(transportPreference||"").trim(),
      emergencyContact:String(emergencyContact||"").trim(),specialRequests:String(specialRequests||"").trim()
    });
...
export const quoteCustomTourRequest=async(req,res,next)=>{try{const {status="quoted",quotedAmount,adminNotes=""}=req.body||{};const amount=Number(quotedAmount);if(!["approved","quoted","rejected"].includes(status))return res.status(400).json({success:false,message:"Invalid request status."});if(status!=="rejected"&&(!Number.isFinite(amount)||amount<=0))return res.status(400).json({success:false,message:"A quoted amount greater than zero is required."});const item=await CustomTourRequest.findOneAndUpdate(mergeTenantFilter({_id:req.params.id}),{status,quotedAmount:status==="rejected"?0:amount,adminNotes:String(adminNotes).trim()},{new:true,runValidators:true}).populate("customer","name email phone");if(!item)return res.status(404).json({success:false,message:"Request not found."});if (item.customer?._id) {
  await Notification.create({recipient:item.customer._id,user:item.customer._id,title:status==="rejected"?"Custom Tour Request Declined":"Custom Tour Quote Ready",message:status==="rejected"?`Your custom tour request was declined. ${adminNotes}`:`Your custom tour request has been priced at KES ${amount.toLocaleString()}. ${adminNotes}`,type:"booking",relatedModel:"CustomTourRequest",relatedId:item._id,actionUrl:"/my-custom-tours"});
}res.json({success:true,request:item})}catch(e){next(e)}};
