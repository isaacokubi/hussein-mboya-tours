...
export const getMyCustomTourRequests=async(req,res,next)=>{try{const requests=await CustomTourRequest.find(mergeTenantFilter(req,{customer:req.user._id}).sort({createdAt:-1}).lean();res.json({success:true,requests})}catch(e){next(e)}};
...
let customDestination = await Destination.findOne(mergeTenantFilter(req,{name:"Custom Tour"});
...
let customTour = await Tour.findOne(mergeTenantFilter(req,{title:"Custom Tour Package"});
