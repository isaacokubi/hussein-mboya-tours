...
bookingSchema.index({ customer: 1, createdAt: -1 }); bookingSchema.index({ agent: 1 }); bookingSchema.index({ travelDate: 1 }); bookingSchema.index({ paymentReference: 1 }); bookingSchema.index({ transactionId: 1 }); bookingSchema.index({ assignedGuide: 1 }); bookingSchema.index({ assignedDriver: 1 }); bookingSchema.index({ assignedVehicle: 1 }); bookingSchema.index({ createdAt: -1 }); bookingSchema.index({ tenantId: 1, externalSource: 1, externalBookingId: 1 }, { unique: true, sparse: true, partialFilterExpression: { externalBookingId: { $gt: "" } } }); bookingSchema.index({ isDeleted: 1 }); bookingSchema.index({ refundStatus: 1 }); bookingSchema.index({ commissionStatus: 1 }); bookingSchema.index({ customer: 1, travelDate: 1 }); bookingSchema.index({ tour: 1, travelDate: 1 }); bookingSchema.index({ tenantId: 1, bookingType: 1, travelDate: 1 }); bookingSchema.index({ tenantId: 1, corporatePin: 1 }); bookingSchema.index({ tenantId: 1, corporateAccount: 1, travelDate: 1 });
