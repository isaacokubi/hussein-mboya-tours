...
const travelerSchema = new firestore.Schema(
  {
    tenantId: { type: firestore.Schema.Types.ObjectId, ref: "Organization", index: true }, name: { type: String, required: true, trim: true, maxlength: 100 }, age: { type: Number, min: 0, max: 120 }, gender: { type: String, enum: ["male", "female", "other"], default: "other" }, passportNumber: { type: String, trim: true, uppercase: true, default: "" }, nationality: { type: String, trim: true, default: "" }, dateOfBirth: Date, emergencyContactName: { type: String, trim: true, default: "" }, emergencyContactPhone: { type: String, trim: true, default: "" }, dietaryRequirements: { type: String, default: "" }, medicalConditions: { type: String, default: "" },
  }, { _id: false }
);
const customerSnapshotSchema = new firestore.Schema({ name: String, email: String, phone: String }, { _id: false });
const emergencyContactSchema = new firestore.Schema({ name: String, phone: String, relationship: String }, { _id: false });
const billingContactSchema = new firestore.Schema({ name: String, email: String, phone: String }, { _id: false });

const bookingSchema = new firestore.Schema(
  {
    tenantId: { type: firestore.Schema.Types.ObjectId, ref: "Organization", index: true }, bookingNumber: { type: String, unique: true, index: true }, customTourRequest: { type: firestore.Schema.Types.ObjectId, ref: "CustomTourRequest", default: null }, customer: { type: firestore.Schema.Types.ObjectId, ref: "Customer", required: false, index: true }, user: { type: firestore.Schema.Types.ObjectId, ref: "User", default: null, index: true }, customerSnapshot: customerSnapshotSchema, contact: { name: { type: String, trim: true, default: "" }, email: { type: String, lowercase: true, trim: true, default: "" }, phone: { type: String, trim: true, default: "" } }, agent: { type: firestore.Schema.Types.ObjectId, ref: "Agent", default: null }, bookingSource: { type: String, enum: ["website", "mobile_app", "agent", "admin", "walk_in", "partner", "api"], default: "website" }, externalSource: { type: String, trim: true, default: undefined }, externalBookingId: { type: String, trim: true, default: undefined }, integrationKeyId: { type: firestore.Schema.Types.ObjectId, ref: "WebsiteIntegrationKey", default: null }, externalMetadata: { type: firestore.Schema.Types.Mixed, default: {} }, bookingType: { type: String, enum: ["individual", "group", "corporate"], default: "individual", index: true }, groupReference: { type: String, trim: true, default: "" }, corporateAccount: { type: firestore.Schema.Types.ObjectId, ref: "CorporateAccount", default: null, index: true }, corporateCompanyName: { type: String, trim: true, default: "" }, corporatePin: { type: String, trim: true, uppercase: true, default: "" }, purchaseOrderNumber: { type: String, trim: true, default: "" }, paymentTerms: { type: String, enum: ["immediate", "deposit", "credit", "staged"], default: "immediate" }, billingContact: billingContactSchema, roomingListReference: { type: String, trim: true, default: "" }, tour: { type: firestore.Schema.Types.ObjectId, ref: "Tour", required: false, default: null, index: true }, travelDate: { type: Date, required: true }, originalTravelDate: { type: Date, default: null }, rescheduleHistory: [{ fromDate: { type: Date }, toDate: { type: Date }, reason: { type: String, default: "" }, requestedAt: { type: Date, default: Date.now } }], rescheduleCount: { type: Number, default: 0, min: 0 }, travelers: { type: [travelerSchema], default: [] }, numberOfGuests: { type: Number, default: 1, min: 1 }, pickupLocation: { type: String, trim: true, default: "" }, pickupTime: Date, hotelName: { type: String, trim: true, default: "" }, roomNumber: { type: String, trim: true, default: "" }, emergencyContact: emergencyContactSchema, specialRequests: [{ type: String, trim: true }], assignedGuide: { type: firestore.Schema.Types.ObjectId, ref: "Staff", default: null }, assignedDriver: { type: firestore.Schema.Types.ObjectId, ref: "Staff", default: null }, assignedVehicle: { type: firestore.Schema.Types.ObjectId, ref: "Vehicle", default: null }, assigned: { type: Boolean, default: false }, subtotal: { type: Number, default: 0, min: 0 }, discountAmount: { type: Number, default: 0, min: 0 }, taxAmount: { type: Number, default: 0, min: 0 }, serviceFee: { type: Number, default: 0, min: 0 }, couponUsed: { type: String, trim: true, default: "" }, totalAmount: { type: Number, required: true, min: 0 }, commissionRate: { type: Number, default: 0, min: 0, max: 100 }, commissionAmount: { type: Number, default: 0, min: 0 }, commissionStatus: { type: String, enum: ["pending", "approved", "paid", "cancelled"], default: "pending" }, commissionPaidAt: { type: Date, default: null }, depositAmount: { type: Number, default: 0, min: 0 }, amountPaid: { type: Number, default: 0, min: 0 }, balanceAmount: { type: Number, default: 0, min: 0 }, paymentMethod: { type: String, enum: ["MPESA", "CARD", "PAYPAL", "BANK_TRANSFER", "CASH"], default: "MPESA" }, paymentStatus: { type: String, enum: ["pending", "partial", "paid", "failed", "cancelled", "refunded"], default: "pending", index: true }, transactionId: { type: String, trim: true }, paymentReference: { type: String, trim: true }, mpesaReceipt: { type: String, trim: true, default: "" }, payments: [{ type: firestore.Schema.Types.ObjectId, ref: "Payment" }], refundAmount: { type: Number, default: 0, min: 0 }, refundStatus: { type: String, enum: ["none", "requested", "approved", "processing", "completed", "rejected"], default: "none" }, refundReason: { type: String, default: "" }, status: { type: String, enum: ["pending", "confirmed", "assigned", "ongoing", "completed", "cancelled", "refunded"], default: "pending", index: true }, cancellationReason: { type: String, default: "" }, cancelledAt: { type: Date, default: null }, confirmedAt: { type: Date, default: null }, completedAt: { type: Date, default: null }, notes: { type: String, default: "" }, documents: [{ name: String, url: String, type: String, uploadedAt: { type: Date, default: Date.now } }], createdBy: { type: firestore.Schema.Types.ObjectId, ref: "User", default: null }, updatedBy: { type: firestore.Schema.Types.ObjectId, ref: "User", default: null }, isDeleted: { type: Boolean, default: false, index: true },
  },
  { timestamps: true, toJSON: { virtuals: true }, toObject: { virtuals: true } }
);

bookingSchema.pre("save", function(next) {
  if (!Number.isFinite(Number(this.amountPaid)) || Number(this.amountPaid) < 0) this.amountPaid = 0;
  if (Number.isFinite(Number(this.totalAmount))) this.balanceAmount = Math.max(0, Number(this.totalAmount) - Number(this.amountPaid || 0));
...
bookingSchema.post("save", async function() {
  ...
  try {
    ...
    const PaymentModel = firestore.models.Payment;
    const CorporateAccountModel = firestore.models.CorporateAccount;
...
const Booking = firestore.models.Booking || firestore.model("Booking", tenantBookingSchema);
