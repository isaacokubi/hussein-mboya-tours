...
journalEntrySchema.pre("validate", function(next) {
  if (!this.lines.length) return next(new Error("Journal entry requires at least two lines."));
  const totals = this.lines.reduce((acc, line) => {
    line.debit = Math.round(Number(line.debit || 0) * 100) / 100;
    line.credit = Math.round(Number(line.credit || 0) * 100) / 100;
    if (line.debit > 0 && line.credit > 0) return acc;
    acc.debit += line.debit; acc.credit += line.credit;
    return acc;
  }, { debit: 0, credit: 0 });
  if (this.lines.some((line) => line.debit > 0 && line.credit > 0)) return next(new Error("A journal line cannot contain both debit and credit."));
  if (this.status === "posted" && Math.abs(totals.debit - totals.credit) > 0.01) return next(new Error("Posted journal entry must balance: debits must equal credits."));
  next();
});

journalEntrySchema.pre("save", function(next) {
  if (!this.entryNumber) this.entryNumber = `JE-${Date.now()}-${Math.floor(Math.random() * 10000)}`;
  next();
});
