...
const journalLineSchema = new firestore.Schema({ account: { type: firestore.Schema.Types.ObjectId, ref: "ChartOfAccount", required: true }, description: { type: String, trim: true, default: "" }, debit: { type: Number, min: 0, default: 0 }, credit: { type: Number, min: 0, default: 0 } }, { _id: true });
const journalEntrySchema = new firestore.Schema({ tenantId: { type: firestore.Schema.Types.ObjectId, ref: "Organization", required: true, index: true }, entryNumber: { type: String, trim: true }, entryDate: { type: Date, default: Date.now, index: true }, description: { type: String, required: true, trim: true, maxlength: 300 }, reference: { type: String, trim: true, default: "" }, sourceType: { type: String, trim: true, default: "manual" }, sourceId: { type: firestore.Schema.Types.ObjectId, default: null }, status: { type: String, enum: ["draft", "posted", "void"], default: "draft", index: true }, lines: { type: [journalLineSchema], default: [] }, postedAt: { type: Date, default: null }, postedBy: { type: firestore.Schema.Types.ObjectId, ref: "User", default: null }, createdBy: { type: firestore.Schema.Types.ObjectId, ref: "User", default: null } }, { timestamps: true });
...
export default firestore.models.JournalEntry || firestore.model("JournalEntry", journalEntrySchema);
