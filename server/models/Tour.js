...
tourSchema.pre("validate", async function (next) {
  if (this.title && (!this.slug || this.isModified("title"))) {
    ...
    let slug = baseSlug;
    let counter = 1;
    while (await mongoose.models.Tour.findOne({ slug, _id: { $ne: this._id } })) {
      counter += 1;
...
tourSchema.pre("validate", function (next) {
  ...
  const days = Math.max(1, Number(this.durationDetails?.days || this.duration || 1));
  if (start) {
...
tourSchema.index({ featured: 1, available: 1 });
tourSchema.index({ published: 1 });
tourSchema.index({ createdBy: 1 });
tourSchema.index({ category: 1 });
tourSchema.index({ country: 1 });
tourSchema.index({ price: 1 });
tourSchema.index({ averageRating: -1 });
tourSchema.index({ totalBookings: -1 });
tourSchema.index({ popularity: -1 });
tourSchema.index({ assignedGuide: 1 });
tourSchema.index({ assignedDriver: 1 });
tourSchema.index({ assignedVehicle: 1 });
tourSchema.index({ assignmentStatus: 1 });
