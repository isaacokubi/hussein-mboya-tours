...
export const getEffectivePermissions = async (user) => {
  ...
  if (roleName === "admin") {
    permissions.push(...ADMIN_COMPATIBILITY_PERMISSIONS.map(normalizePermission));
  }
...
export const authorize = (permission) => async (req, res, next) => {
  try {
    ...
    if (!wanted) {
      return res.status(500).json({ success: false, message: "Authorization configuration error" });
    }
