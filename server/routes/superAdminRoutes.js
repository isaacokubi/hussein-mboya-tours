...
router.post("/tenants/:id/subscription/activate", authorize("user.manage"), approveTenantSubscription);
router.delete("/tenants/:id", authorize("user.manage"), deleteTenant);
router.get("/subscription-payments", authorize("user.manage"), listSubscriptionPayments);
...
router.get("/security", authorize("system.security"), async (req, res) => { try { const data = await securityService.getSecurityStatus(); return res.json({ success: true, data }); } catch (error) { console.error("SuperAdmin security status error:", error); return res.status(500).json({ success: false, message: error.message }); } });
