...
function buildModel(name,schema){
  ...
  Model.updateMany=async(filter,update,options={})=>{const docs=await Model.find(filter).session(options.session);for(const d of docs){await applyUpdate(d,update,options);await d.save({session:options.session});}return {matchedCount:docs.length,modifiedCount:docs.length};};
  Model.findOneAndUpdate=async(filter,update,options={})=>{let d=await Model.findOne(filter).session(options.session);if(!d&&options.upsert)d=new Model({...filter,...(update.$setOnInsert||{}),...(update.$set||{})});if(!d)return null;await applyUpdate(d,update,options);await d.save({session:options.session});return d;};
