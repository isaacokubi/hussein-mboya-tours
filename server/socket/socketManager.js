...
export const initSocket = (socketServer) => {
  ...
  io.use(async (socket, next) => {
    try {
      ...
      if (role !== "superadmin" && !tenantId) return next(new Error("Authentication tenant is required"));
      ...
      if (role !== "superadmin" && String(user.tenantId || "") !== tenantId) return next(new Error("Authentication tenant mismatch"));
