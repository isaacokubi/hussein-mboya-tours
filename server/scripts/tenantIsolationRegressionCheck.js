...
const suffix = `1790164836382-${Math.random().toString(36).slice(2, 8)}`;
const createdTenantIds = [];
const createdDestinationIds = [];
const createdAccommodationIds = [];
const results = [];

const fixture = (name, slug) => ({
  name,
  slug,
  country: "Kenya",
  description: `Tenant isolation regression fixture for ${name}.`,
});
...
try {
  const tenantA = await runWithTenant({ bypass: true }, () =>
    Organization.create({
  ...
  const tenantB = await runWithTenant({ bypass: true }, () =>
    Organization.create({
  ...
  await expect("Tenant A create", async () => {
    destinationA = await runWithTenant({ tenantId: tenantA._id }, () =>
      Destination.create(
        fixture(
          `Isolation Destination A ${suffix}`,
          `isolation-destination-a-${suffix}`,
        ),
      ),
    );
  ...
  await expect("Tenant-scoped accommodation inventory create", async () => {
    const inventory = await runWithTenant({ tenantId: tenantA._id }, () =>
    ...
    if (String(inventory.tenantId) !== String(tenantA._id)) {
      throw new Error("Accommodation inventory was not assigned to Tenant A.");
    }
  });

  await expect("Cross-tenant accommodation read blocked", async () => {
    const inventory = await runWithTenant({ tenantId: tenantB._id }, () =>
      AccommodationInventory.findById(createdAccommodationIds[0]).lean(),
    );
  ...
  await expect("Cross-tenant accommodation update blocked", async () => {
    const result = await runWithTenant({ tenantId: tenantB._id }, () =>
      AccommodationInventory.updateOne(
    ...
    if (result.matchedCount !== 0) {
      throw new Error("Tenant B updated Tenant A accommodation inventory.");
    }
  });

  await expect("Tenant B create", async () => {
    destinationB = await runWithTenant({ tenantId: tenantB._id }, () =>
      Destination.create(
        fixture(
          `Isolation Destination B ${suffix}`,
          `isolation-destination-b-${suffix}`,
        ),
      ),
    );
  ...
  await expect("Missing tenant context fails closed", async () => {
    try {
      await Destination.findOne({ _id: destinationA._id }).lean();
    } catch (error) {
  ...
  await expect("Cross-tenant read blocked", async () => {
    const found = await runWithTenant({ tenantId: tenantA._id }, () =>
      Destination.findById(destinationB._id).lean(),
    );
  ...
  await expect("Cross-tenant update blocked", async () => {
    const result = await runWithTenant({ tenantId: tenantA._id }, () =>
      Destination.updateOne(
        { _id: destinationB._id },
        { $set: { name: `ATTACK ${suffix}` } },
      ),
    );
  ...
  await expect("Cross-tenant delete blocked", async () => {
    const result = await runWithTenant({ tenantId: tenantA._id }, () =>
      Destination.deleteOne({ _id: destinationB._id }),
    );
  ...
  await expect("Tenant-scoped insertMany", async () => {
    const docs = await runWithTenant({ tenantId: tenantA._id }, () =>
    ...
    if (docs.some((doc) => String(doc.tenantId) !== String(tenantA._id))) {
      throw new Error("insertMany assigned the wrong tenant.");
    }
  });

  await expect("Tenant data survives cross-tenant attacks", async () => {
    const own = await runWithTenant({ tenantId: tenantA._id }, () =>
      Destination.findById(destinationA._id).lean(),
    );
    const other = await runWithTenant({ tenantId: tenantB._id }, () =>
      Destination.findById(destinationA._id).lean(),
    );
  ...
  await runWithTenant({ bypass: true }, async () => {
    if (createdDestinationIds.length) {
      await Destination.deleteMany({ _id: { $in: createdDestinationIds } });
    }
    if (createdAccommodationIds.length) {
      await AccommodationInventory.deleteMany({ _id: { $in: createdAccommodationIds } });
    }
    if (createdTenantIds.length) {
      await Organization.deleteMany({ _id: { $in: createdTenantIds } });
    }
  });
}
