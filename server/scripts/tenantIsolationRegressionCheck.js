...
try {
  const tenantA = await runWithTenant({ bypass: true }, () => Organization.create({ name: `Isolation A ${suffix}`, slug: `isolation-a-${suffix}`,
      domain: `tenant-a-${suffix}.test`,
      status: "active" }));
  const tenantB = await runWithTenant({ bypass: true }, () => Organization.create({ name: `Isolation B ${suffix}`, slug: `isolation-b-${suffix}`,
  ...
  await expect("Cross-tenant read blocked", async () => {
    const found = await runWithTenant({ tenantId: tenantA._id }, async () => await Destination.findById(destinationB._id).lean());
    if (found) throw new Error("Tenant A read Tenant B data.");
  });

  await expect("Cross-tenant update blocked", async () => {
    const result = await runWithTenant({ tenantId: tenantA._id }, async () => await Destination.updateOne({ _id: destinationB._id }, { $set: { name: `ATTACK ${suffix}` } }));
    if (result.matchedCount !== 0) throw new Error("Tenant A updated Tenant B data.");
  });

  await expect("Cross-tenant delete blocked", async () => {
    const result = await runWithTenant({ tenantId: tenantA._id }, async () => await Destination.deleteOne({ _id: destinationB._id }));
    if (result.deletedCount !== 0) throw new Error("Tenant A deleted Tenant B data.");
  });

  await expect("Tenant-scoped insertMany", async () => {
    const docs = await runWithTenant({ tenantId: tenantA._id }, async () => await Destination.insertMany([
  ...
  await expect("Tenant-scoped bulkWrite", async () => {
    const result = await runWithTenant({ tenantId: tenantA._id }, async () => await Destination.bulkWrite([
  ...
  await expect("Cross-tenant aggregation lookup blocked", async () => {
    const rows = await runWithTenant({ tenantId: tenantA._id }, async () => await Destination.aggregate([
  ...
  await expect("estimatedDocumentCount fails closed", async () => {
    try { await runWithTenant({ tenantId: tenantA._id }, async () => await Destination.estimatedDocumentCount()); }
    catch (error) {
      if (/not tenant-safe|Tenant context|blocked for tenant-scoped models|countDocuments/i.test(error.message)) return;
      throw error;
    }
  ...
  await expect("Tenant data survives cross-tenant attacks", async () => {
    const own = await runWithTenant({ tenantId: tenantA._id }, async () => await Destination.findById(destinationA._id).lean());
    const other = await runWithTenant({ tenantId: tenantB._id }, async () => await Destination.findById(destinationA._id).lean());
...
console.log("\nTenant Isolation Regression Results");
console.log("================================");
