...
function convertUniquePathToTenantIndex(schema, path) {
  if (!path?.options?.unique || path.path === TENANT_PATH) return;

  const field = path.path;
  const sparse = Boolean(path.options.sparse);

  // A tenant-owned field must be unique inside a tenant, not globally.
  // Clear both path-level flags so schema.indexes() cannot recreate the
  // obsolete global unique index on application startup.
  path.options.unique = false;
  path.options.index = false;
  if (typeof path.index === "function") path.index(false);
...
export function tenantPlugin(schema) {
  ...
  for (const path of Object.values(schema.paths || {})) {
    convertUniquePathToTenantIndex(schema, path);
  }
      ...
      try {
        ...
        if (["findOneAndUpdate", "updateOne", "updateMany", "replaceOne"].includes(hook) && shouldScope()) {
          ...
          if (update && !Array.isArray(update)) {
            ...
            if (requestedTenant && String(requestedTenant) !== String(tenantId)) {
