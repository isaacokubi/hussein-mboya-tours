...
const EVENT_TYPES = Object.freeze({
  ...
  SUBSCRIPTION_7: "subscription_7_days",
  SUBSCRIPTION_3: "subscription_3_days",
  SUBSCRIPTION_1: "subscription_1_day",
  SUBSCRIPTION_EXPIRED: "subscription_expired",
});

const normalizePhone = (phone) => String(phone || "").trim();
const bookingContact = (booking) => ({
  name: booking.customerSnapshot?.name || booking.contact?.name || "Customer",
  email: booking.customerSnapshot?.email || booking.contact?.email || "",
  phone: normalizePhone(booking.customerSnapshot?.phone || booking.contact?.phone),
});
...
const recordInApp = async ({ tenantId, recipient, title, message, type, eventKey, relatedId }) => {
  ...
  return runWithTenant({ tenantId, role: "system" }, () => Notification.create({
    tenantId,
    recipient,
    user: recipient,
    title,
    message,
    type,
    metadata: { eventKey, automation: true },
    relatedModel: relatedId ? "Booking" : null,
    relatedId: relatedId || null,
    isSent: true,
  }));
};

const deliver = async ({ booking, eventKey, title, message, emailSubject, html, type = "booking" }) => {
  ...
  const tasks = [];
  if (contact.email) tasks.push(sendEmail({ to: contact.email, subject: emailSubject, html, text: message }));
  ...
  if (!results.length) return { skipped: true, reason: "No customer communication channel configured", eventKey };
...
export const sendBookingConfirmationAutomation = async (booking) => {
  ...
  return deliver({
    ...
    message: `Hello ${name}, your booking ${booking.bookingNumber || ""} with ${companyName} is confirmed for ${formatDate(booking.travelDate)}.`,
    emailSubject: `Booking Confirmation - ${companyName}`,
    html: `<h2>Booking Confirmed</h2><p>Hello ${escapeHtml(name)},</p><p>Your booking <strong>${escapeHtml(booking.bookingNumber || "Pending")}</strong> is confirmed.</p><p><strong>Travel date:</strong> ${escapeHtml(formatDate(booking.travelDate))}</p><p><strong>Total:</strong> ${escapeHtml(booking.totalAmount ?? booking.amount ?? "0")}.</p><p>Thank you for choosing ${escapeHtml(companyName)}.</p>`,
    type: "booking",
  });
};
...
export const sendPaymentReminderAutomation = async (booking) => {
  ...
  return deliver({
    ...
    message: `Hello ${name}, this is a payment reminder for booking ${booking.bookingNumber || ""}. Please contact ${companyName} if you need assistance.`,
    emailSubject: `Payment Reminder - ${booking.bookingNumber || "Booking"}`,
    html: `<h2>Payment Reminder</h2><p>Hello ${escapeHtml(name)},</p><p>This is a reminder that payment is still pending for booking <strong>${escapeHtml(booking.bookingNumber || "")}</strong>.</p><p>Travel date: ${escapeHtml(formatDate(booking.travelDate))}</p><p>Please contact ${escapeHtml(companyName)} if you need assistance.</p>`,
    type: "payment",
  });
};
...
export const sendTourReminderAutomation = async (booking) => {
  ...
  return deliver({
    ...
    eventKey,
    title: "Tour Reminder",
    message: `Hello ${name}, your ${booking.tour?.title || "tour"} with ${companyName} is coming up on ${formatDate(booking.travelDate)}. We look forward to welcoming you.`,
    emailSubject: `Tour Reminder - ${booking.bookingNumber || "Booking"}`,
    html: `<h2>Your Tour Is Coming Up</h2><p>Hello ${escapeHtml(name)},</p><p>Your <strong>${escapeHtml(booking.tour?.title || "tour")}</strong> is scheduled for <strong>${escapeHtml(formatDate(booking.travelDate))}</strong>.</p><p>We look forward to welcoming you.</p>`,
    type: "tour_update",
  });
};
...
export const sendTourVoucherAutomation = async (booking) => {
  ...
  return deliver({
    booking,
    eventKey,
    title: "Tour Voucher Ready",
    message: `Your tour voucher for booking ${booking.bookingNumber || ""} is ready. Please keep your booking number available when travelling.`,
    emailSubject: `Tour Voucher - ${booking.bookingNumber || "Booking"}`,
    html: `<h2>Tour Voucher</h2><p>Hello ${escapeHtml(name)},</p><p>Your tour voucher is ready.</p><p><strong>Booking:</strong> ${escapeHtml(booking.bookingNumber || "")}</p><p><strong>Tour:</strong> ${escapeHtml(booking.tour?.title || "Tour")}</p><p><strong>Travel date:</strong> ${escapeHtml(formatDate(booking.travelDate))}</p><p>Regards,<br>${escapeHtml(companyName)}</p>`,
    type: "booking",
  });
};
...
export const runCommunicationAutomation = async () => {
  ...
  const bookings = await Booking.find({ travelDate: { $gte: now, $lte: threeDays }, status: { $nin: ["cancelled", "completed"] }, isDeleted: { $ne: true } }).populate("tour", "title").lean();
