...
const account = async (tenantId, code) =>
  ChartOfAccount.findOne({ tenantId, code, active: true }).lean();
...
export async function postFinanceEntry({
  tenantId,
  sourceType,
  sourceId,
  description,
  ...
  if (!Array.isArray(lines) || lines.length < 2) {
    throw new Error("Finance journal entry requires at least two lines.");
  }
  ...
  for (const line of lines) {
    ...
    if (debit < 0 || credit < 0 || (debit > 0 && credit > 0)) {
      throw new Error(`Invalid debit/credit values for account ${line.code}.`);
    }
  ...
  if (debit <= 0 || Math.abs(debit - credit) > 0.01) {
    throw new Error("Finance journal entry is not balanced.");
  }
  ...
  try {
    ...
    if (error?.code === 11000) {
      return JournalEntry.findOne({ tenantId, sourceType, sourceId }).lean();
    }
    throw error;
  }
}

export const postSupplierPayable = (payable) =>
  postFinanceEntry({
    tenantId: payable.tenantId,
    sourceType: "supplier_payable",
    sourceId: payable._id,
    description: `Supplier payable ${payable._id}`,
    ...
    lines: [
      {
        ...
        credit: 0,
        description: "Supplier cost accrued",
      },
      {
        code: "2000",
        debit: 0,
...
export const postSupplierPayment = ({ payable, amount, paymentReference }) => {
  ...
  return postFinanceEntry({
    tenantId: payable.tenantId,
    sourceType: "supplier_payable_payment",
    sourceId,
    description: `Supplier payable settlement ${paymentReference || payable._id}`,
...
export const postCreditDebitNote = (note) => {
  ...
  return postFinanceEntry({
    ...
    sourceId: note._id,
    description: `${credit ? "Credit" : "Debit"} note ${note.noteNumber}`,
    reference: note.noteNumber,
    date: note.issuedAt || new Date(),
    lines,
  });
};
