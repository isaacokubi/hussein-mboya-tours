...
const ensure = async (tenantId) => {
  const defaults = [
    ...
    ["5000", "Tour Direct Costs", "expense", "cost_of_sales"],
  ];
  for (const [code, name, type, subtype] of defaults) {
    await ChartOfAccount.updateOne(
      { tenantId, code },
      { $setOnInsert: { tenantId, code, name, type, subtype, currency: "KES", isActive: true } },
      { upsert: true }
    );
  }
};
...
export async function postFinanceEntry({ tenantId, sourceType, sourceId, description, reference = "", date = new Date(), lines }) {
  ...
  for (const line of lines) {
    ...
    resolved.push({ account: acc._id, description: line.description || description, debit: money(line.debit), credit: money(line.credit) });
  }
  const debit = money(resolved.reduce((s, l) => s + l.debit, 0));
  const credit = money(resolved.reduce((s, l) => s + l.credit, 0));
...
export const postCreditDebitNote = (note) => {
  ...
  const lines = credit
    ? [{ code: "4000", debit: note.amount, credit: 0, description: "Revenue reversal" }, ...(Number(note.taxAmount) > 0 ? [{ code: "2100", debit: note.taxAmount, credit: 0, description: "Tax reversal" }] : []), { code: "1100", debit: 0, credit: note.totalAmount, description: "Accounts receivable credit" }]
    : [{ code: "1100", debit: note.totalAmount, credit: 0, description: "Accounts receivable debit" }, { code: "4000", debit: 0, credit: note.amount, description: "Debit note revenue" }, ...(Number(note.taxAmount) > 0 ? [{ code: "2100", debit: 0, credit: note.taxAmount, description: "Tax payable" }] : [])];
