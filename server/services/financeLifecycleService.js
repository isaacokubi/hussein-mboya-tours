...
const ensure = async (tenantId) => {
  const defaults = [
    ["1000", "Cash on Hand", "asset", "cash"], ["1010", "Bank Account", "asset", "bank"], ["1020", "M-Pesa", "asset", "mobile_money"], ["1030", "Card / Gateway Clearing", "asset", "payment_clearing"], ["1100", "Accounts Receivable", "asset", "receivable"], ["2000", "Accounts Payable", "liability", "payable"], ["2100", "VAT / Tax Payable", "liability", "tax"], ["2110", "Output VAT", "liability", "vat_output"], ["2120", "Input VAT", "asset", "vat_input"], ["2130", "Withholding Tax Payable", "liability", "withholding_tax"], ["4000", "Tour Revenue", "revenue", "sales"], ["5000", "Tour Direct Costs", "expense", "cost_of_sales"],
  ];
...
export async function postFinanceEntry({ tenantId, sourceType, sourceId, description, reference = "", date = new Date(), lines }) {
  ...
  for (const line of lines) {
    ...
    if (!acc) throw new Error(`Account ${line.code} is unavailable.`);
    const debit = money(line.debit); const credit = money(line.credit);
    if (debit < 0 || credit < 0 || (debit > 0 && credit > 0)) throw new Error(`Invalid debit/credit values for account ${line.code}.`);
    resolved.push({ account: acc._id, description: line.description || description, debit, credit });
  }
  const debit = money(resolved.reduce((sum, line) => sum + line.debit, 0));
  const credit = money(resolved.reduce((sum, line) => sum + line.credit, 0));
  if (debit <= 0 || Math.abs(debit - credit) > 0.01) throw new Error("Finance journal entry is not balanced.");
  try {
    return await JournalEntry.create({ tenantId, sourceType, sourceId, entryDate: date, description, reference, status: "posted", lines: resolved, postedAt: new Date() });
  } catch (error) {
...
export const postCreditDebitNote = (note) => {
  ...
  const lines = credit
    ? [{ code: "4000", debit: note.amount, credit: 0, description: "Revenue reversal" }, ...(Number(note.taxAmount) > 0 ? [{ code: "2110", debit: note.taxAmount, credit: 0, description: "Output VAT reversal" }] : []), { code: "1100", debit: 0, credit: note.totalAmount, description: "Accounts receivable credit" }]
    : [{ code: "1100", debit: note.totalAmount, credit: 0, description: "Accounts receivable debit" }, { code: "4000", debit: 0, credit: note.amount, description: "Debit note revenue" }, ...(Number(note.taxAmount) > 0 ? [{ code: "2110", debit: 0, credit: note.taxAmount, description: "Output VAT" }] : [])];
