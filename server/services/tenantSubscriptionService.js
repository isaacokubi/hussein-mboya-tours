...
export const activateTenantSubscription = async ({ tenantId, plan, provider = "mpesa", periodDays = 30, payment, transactionReference = "" }) => {
  ...
  const organization = await Organization.findById(tenantId); if (!organization) throw new Error("Company not found.");
  const now = new Date(); const days = Math.max(1, Number(periodDays) || 30); const end = new Date(now.getTime() + days * 86400000);
  organization.status = "active";
  organization.subscription = { ...(organization.subscription?.toObject?.() || organization.subscription || {}), plan, seats: organization.subscription?.seats || 5, trialEndsAt: organization.subscription?.trialEndsAt || null, renewsAt: end };
  await organization.save();
  await Subscription.findOneAndUpdate({ tenantId: organization._id }, { $set: { tenantId: organization._id, plan, status: "active", provider, currentPeriodStartsAt: now, currentPeriodEndsAt: end, cancelledAt: null, trialStartsAt: organization.createdAt || now, trialEndsAt: organization.subscription?.trialEndsAt || now, seats: organization.subscription?.seats || 5 } }, { upsert: true, new: true, setDefaultsOnInsert: true });
...
export const expireTenantSubscriptions = async () => {
  const now = new Date();
  const orgs = await Organization.find({ status: { $in: ["trial", "active"] }, $or: [{ "subscription.trialEndsAt": { $lte: now } }, { "subscription.renewsAt": { $lte: now } }] }).select("_id").lean();
...
export const initiateTenantMpesaPayment = async ({ tenantId, userId, plan, phone, amount }) => {
  ...
  } catch (error) { payment.status = "failed"; payment.failureReason = error.response?.data?.errorMessage || error.message || "M-Pesa subscription request failed."; await payment.save(); throw new Error(payment.failureReason); }
};
