...
export async function createPurchaseOrder(data, userId) {
  requireTenantId();
  assertObjectId(data.supplier, "Supplier");
  if (data.booking) assertObjectId(data.booking, "Booking");
  if (data.tour) assertObjectId(data.tour, "Tour");
  const supplier = await Supplier.findOne({ ...tenantFilter(), _id: data.supplier, status: "active" });
  if (!supplier) throw new Error("Supplier not found in this tenant or is inactive.");
  if (data.booking && !(await Booking.exists({ ...tenantFilter(), _id: data.booking, isDeleted: { $ne: true } }))) throw new Error("Booking not found in this tenant.");
  if (data.tour && !(await Tour.exists({ ...tenantFilter(), _id: data.tour, isDeleted: { $ne: true } }))) throw new Error("Tour not found in this tenant.");
...
export async function transitionPurchaseOrder(id, status, userId) {
  ...
  const po = await PurchaseOrder.findOne({ ...tenantFilter(), _id: id });
...
export async function createTourCost(data, userId) {
  requireTenantId();
  assertObjectId(data.tour, "Tour");
  const tour = await Tour.findOne({ ...tenantFilter(), _id: data.tour, isDeleted: { $ne: true } });
  if (!tour) throw new Error("Tour not found in this tenant.");
  if (data.booking && !(await Booking.exists({ ...tenantFilter(), _id: data.booking, isDeleted: { $ne: true } }))) throw new Error("Booking not found in this tenant.");
  if (data.supplier && !(await Supplier.exists({ ...tenantFilter(), _id: data.supplier }))) throw new Error("Supplier not found in this tenant.");
  if (data.purchaseOrder && !(await PurchaseOrder.exists({ ...tenantFilter(), _id: data.purchaseOrder }))) throw new Error("Purchase order not found in this tenant.");
  return TourCost.create({ ...data, createdBy: userId });
}

export async function getTourProfitability(tourId) {
  requireTenantId();
  assertObjectId(tourId, "Tour");
...
export async function createSupplierPayable(data, userId) {
  requireTenantId();
  assertObjectId(data.supplier, "Supplier");
  if (!(await Supplier.exists({ ...tenantFilter(), _id: data.supplier }))) throw new Error("Supplier not found in this tenant.");
  for (const [key, label, Model] of [["purchaseOrder", "Purchase order", PurchaseOrder], ["expense", "Expense", null], ["booking", "Booking", Booking], ["tour", "Tour", Tour]]) {
    if (data[key] && Model && !(await Model.exists({ ...tenantFilter(), _id: data[key] }))) throw new Error(`${label} not found in this tenant.`);
  }
...
export async function paySupplierPayable(id, amount, paymentReference = "") {
  requireTenantId();
  const payable = await SupplierPayable.findOne({ ...tenantFilter(), _id: id });
  ...
  if (payment > payable.balance) throw new Error("Payment exceeds supplier payable balance.");
  payable.amountPaid = money(payable.amountPaid + payment);
  payable.paymentReference = paymentReference || payable.paymentReference;
  return payable.save();
}

export async function createCorporateAccount(data, userId) {
  requireTenantId();
  return CorporateAccount.create({ ...data, createdBy: userId });
}
...
export async function getResourceConflicts({ resourceType, resourceId, travelDate, excludeBookingId }) {
  ...
  if (!travelDate) throw new Error("travelDate is required.");
  const start = new Date(travelDate); if (Number.isNaN(start.getTime())) throw new Error("travelDate is invalid.");
  start.setHours(0, 0, 0, 0); const end = new Date(start); end.setDate(end.getDate() + 1);
  const field = { vehicle: "assignedVehicle", guide: "assignedGuide", driver: "assignedDriver" }[resourceType];
  if (!field) throw new Error("resourceType must be vehicle, guide, or driver.");
  const query = { ...tenantFilter(), [field]: resourceId, travelDate: { $gte: start, $lt: end }, status: { $nin: ["cancelled", "refunded", "completed"] }, isDeleted: { $ne: true } };
