  ...
  try {
    const data = await runWithTenant(
      ...
      async () => {
        ...
        const [users, staff, agents, vehicles, bookings, admins, tours, destinations, payments, revenue] = await Promise.all([
          User.countDocuments(),
        ...
        return {
          ...
          vehicles,
          bookings,
          admins, tours, destinations, payments, revenue: revenue?.[0]?.total || 0,
        };
      }
    );
