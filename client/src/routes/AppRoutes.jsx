...
function SuperAdminProtected({ children }) {
  return <ProtectedRoute roles={["super_admin", "superadmin"]}>{children}</ProtectedRoute>;
}
...
export default function AppRoutes() {
  return (
    <>
      <ScrollToTop />
      <Suspense fallback={<div style={{ minHeight: "50vh", display: "flex", alignItems: "center", justifyContent: "center", fontSize: "1rem" }}>Loading...</div>}>
        <Routes>
          ...
          <Route path="/tour-manager" element={<ProtectedRoute roles={["manager", "admin"]}><TourManagerLayout /></ProtectedRoute>}>
