...
import { Activity, BarChart3, Bell, Bot, Car, CalendarCheck, CreditCard, FileBarChart, FileText, Handshake, Image, LayoutDashboard, Map, Menu, PackageCheck, PlusCircle, Receipt, Settings, Shield, Smartphone, Star, Tag, UserCog, Users, Wallet, X } from "lucide-react";
...
const MENU = [
  ...
  ["Operations & Procurement", "/admin/operations", PackageCheck, "booking.manage"], ["Payments", "/admin/payments", CreditCard, "payment.manage"],
  ...
  ["Roles & Permissions", "/admin/rbac", Shield, "roles.manage"], ["System Health", "/admin/system-health", Activity, "admin.dashboard"], ["Billing & Subscription", "/admin/billing", CreditCard, "settings.manage"], ["Settings", "/admin/settings", Settings, "settings.manage"],
];
...
export default function AdminLayout() {
  ...
  return <div className="dashboard-responsive min-h-screen bg-slate-100">
    ...
    <div className="min-h-screen lg:pl-72"><header className="sticky top-0 z-40 flex items-center justify-between border-b bg-white/95 px-3 py-2.5 shadow-sm backdrop-blur sm:px-4 sm:py-3 lg:px-8"><div className="flex min-w-0 items-center gap-2 sm:gap-3"><button onClick={() => setMobileOpen(true)} className="shrink-0 rounded-xl bg-slate-900 p-2 text-white lg:hidden" aria-label="Open admin menu"><Menu size={21} /></button><div className="min-w-0"><p className="truncate text-[10px] font-semibold uppercase tracking-wider text-emerald-700 sm:text-xs">Administration</p><h1 className="truncate text-sm font-bold text-slate-900 sm:text-base lg:text-lg">{companyName} Control Center</h1></div></div><div className="hidden max-w-[35%] text-right sm:block"><p className="truncate text-sm font-semibold text-slate-900">{user?.name || "Administrator"}</p><p className="text-[10px] capitalize text-slate-500 sm:text-xs">{String(role || "admin").replace(/_/g, " ")}</p></div></header><main className="min-w-0 p-3 sm:p-4 md:p-6 lg:p-8"><Outlet /></main></div>
  </div>;
}
