...
const TourManagerLayout = () => {
  ...
  // Admin accounts have their own control center. The route tree historically
  // allowed admins into this layout, which could make an admin session appear
  // to be a Tour Manager after a deep-link or stale navigation. Fail closed.
      ...
      {mobileOpen && (
        <div className="ops-drawer open lg:hidden" role="dialog" aria-modal="true" aria-label="Tour Manager navigation">
          ...
          <aside className="ops-drawer-panel">
            <TourManagerSidebar />
            <button type="button" onClick={() => setMobileOpen(false)} className="absolute right-3 top-3 rounded-xl bg-white/10 p-2 text-white transition hover:bg-white/20" aria-label="Close navigation">
              <X size={21} />
            </button>
          </aside>
        </div>
      ...
      <div className="ops-main flex min-w-0 flex-1 flex-col">
        <header className="ops-header lg:hidden">
          <div className="flex items-center gap-3">
            <button type="button" onClick={() => setMobileOpen(true)} className="ops-mobile-trigger" aria-label="Open Tour Manager navigation" aria-expanded={mobileOpen}>
              <Menu size={22} />
            </button>
            <div>
