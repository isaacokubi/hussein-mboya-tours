...
export const createHotelBooking = async (payload) => {
  ...
  if (!booking?._id) { const error = new Error(body?.message || "The hotel reservation was not created."); error.response = response; throw error; }
...
export const getAdminHotels = async (params = {}) => { await syncHotelInventory().catch(() => null); return api.get("/hotels/admin/catalog", { params }).then(unwrap).then(tenantSafe); };
