...
useEffect(()=>{
 if(!selected && roles.length){
   ...
   setSelected(
   superRole?._id || superRole?.id ||
   roles[0]?._id || roles[0]?.id
 );
 }
},[roles,selected]);
...
function PermissionEditor({role,grouped,mutation}) {
  const initial = new Set(
    (role?.permissions || []).map((p) => p._id || p.id)
);
