...
<textarea rows="6" placeholder="Accommodation, meals, transfers, activities, children, accessibility, special occasions and anything else..." value={form.requirements} onChange={e=>setForm({...form,requirements:e.target.value})} className="w-full rounded-xl border p-3"/><button disabled={mutation.isPending} className="rounded-xl bg-emerald-700 px-6 py-3 font-bold text-white">{mutation.isPending?"Submitting...":"Request my custom tour"}</button></form><div className="mt-8 space-y-4"><h2 id="my-requests" className="text-2xl font-bold">My requests</h2>{(data?.requests||[]).map(r=><div key={r._id} className="rounded-2xl bg-white p-5 shadow-sm ring-1 ring-slate-200"><div className="flex flex-wrap justify-between gap-3"><div><b>{r.destination}</b><div className="text-sm text-slate-500">{r.durationDays} days · {r.people} people</div></div><span className="rounded-full bg-slate-100 px-3 py-1 text-xs font-bold capitalize">{r.status}</span></div>{r.status==="quoted" && Number(r.quotedAmount) > 0 && (
<button
onClick={async()=>{
try{
