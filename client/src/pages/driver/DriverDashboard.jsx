...
export default function DriverDashboard() {
  ...
  const loadDashboard = async (background = false) => {
    try {
      ...
      const tourPayload = unwrap(toursResponse);
      setAssignedTours(Array.isArray(tourPayload?.tours) ? tourPayload.tours : Array.isArray(tourPayload) ? tourPayload : []);
    } catch (err) {
      setError(err?.response?.data?.message || err?.message || "Unable to load your driver operations.");
    } finally {
      setLoading(false);
      setRefreshing(false);
    }
  ...
  const changeStatus = async (tour, status) => {
    ...
    try {
      ...
      setError(err?.response?.data?.message || err?.message || "Unable to update tour status.");
    } finally {
      setActionId("");
    }
  };
  ...
  return (
    <div className="ops-page">
      <div className="ops-page-head">
        <div><div className="ops-page-title">Driver Operations</div><div className="ops-page-subtitle">{dashboard?.driver?.name || dashboard?.profile?.name || "Your"} — assignments, pickups and vehicle readiness.</div></div>
        <button className="btn btn-secondary" type="button" onClick={() => loadDashboard(true)} disabled={refreshing}><RefreshCw size={15} className={refreshing ? "spin" : ""} /> Refresh</button>
      </div>
