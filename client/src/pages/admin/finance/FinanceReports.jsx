...
const Card = ({ title, value, hint, icon: Icon = Wallet }) => (
  <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
    <div className="flex items-start justify-between gap-3">
      <div><p className="text-sm font-medium text-slate-500">{title}</p><p className="mt-2 text-2xl font-bold tracking-tight text-slate-900">{value}</p></div>
      <span className="rounded-xl bg-emerald-50 p-2.5 text-emerald-700"><Icon size={20} /></span>
    </div>
...
export default function FinanceReports() {
  ...
  return <div className="min-h-full space-y-6 bg-slate-50 p-4 md:p-6">
    ...
    {hasReview && <div className="flex gap-3 rounded-xl border border-amber-300 bg-amber-50 p-4 text-sm text-amber-900"><AlertTriangle className="mt-0.5 shrink-0" size={19} /><div><p className="font-semibold">Accounting control review required</p><p className="mt-1">At least one control-account reconciliation or balance-sheet check is outside tolerance. Investigate the variance before period close.</p></div></div>}
    ...
    <section className="grid gap-6 lg:grid-cols-2"><div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm"><h2 className="text-xl font-semibold text-slate-900">Accounts Receivable Aging</h2><p className="mb-4 text-sm text-slate-500">Outstanding customer and corporate invoices from the operational invoice ledger.</p><Aging buckets={arData.buckets} /><p className="mt-4 text-right font-bold text-slate-900">Outstanding: {money(arData.totalOutstanding)}</p></div><div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm"><h2 className="text-xl font-semibold text-slate-900">Accounts Payable Aging</h2><p className="mb-4 text-sm text-slate-500">Outstanding supplier liabilities from the supplier-payable ledger.</p><Aging buckets={apData.buckets} /><p className="mt-4 text-right font-bold text-slate-900">Outstanding: {money(apData.totalOutstanding)}</p></div></section>
    <section className="rounded-2xl border border-slate-200 bg-white shadow-sm"><div className="border-b border-slate-100 p-5"><h2 className="text-xl font-semibold text-slate-900">Cash Flow</h2><p className="text-sm text-slate-500">Posted movements through cash, bank, M-Pesa and gateway-clearing accounts. This is cash movement, not revenue.</p></div><div className="grid gap-4 p-5 md:grid-cols-3"><Card title="Inflows" value={money(cashData.inflows)} /><Card title="Outflows" value={money(cashData.outflows)} /><Card title="Net movement" value={money(cashData.netMovement)} /></div><div className="overflow-x-auto px-5 pb-5"><table className="w-full text-sm"><thead className="bg-slate-50"><tr><th className="p-3 text-left">Date</th><th className="p-3 text-left">Account</th><th className="p-3 text-left">Description</th><th className="p-3 text-right">Movement</th></tr></thead><tbody>{(Array.isArray(cashData.movements) ? cashData.movements : []).slice(-100).reverse().map((m, i) => <tr key={`${m.date || "movement"}-${i}`} className="border-t border-slate-100 hover:bg-slate-50/70"><td className="p-3">{m.date ? new Date(m.date).toLocaleDateString() : "Unavailable"}</td><td className="p-3 font-mono text-xs">{m.accountCode || "Unavailable"}</td><td className="p-3">{m.description || "Unavailable"}{m.reference && /DEMO|TEST/i.test(m.reference) && <span className="ml-2 rounded-full border border-amber-200 bg-amber-50 px-2 py-0.5 text-[10px] font-semibold text-amber-800">TEST/DEMO</span>}</td><td className={`p-3 text-right font-medium ${numericValue(m.amount) !== null && Number(m.amount) >= 0 ? "text-emerald-700" : "text-red-700"}`}>{money(m.amount)}</td></tr>)}</tbody></table>{!cashData.movements?.length && !cash.isLoading && <p className="py-8 text-center text-sm text-slate-500">No posted cash movements are available for this period.</p>}</div></section>
    <section className="rounded-2xl border border-slate-200 bg-white shadow-sm"><div className="border-b border-slate-100 p-5"><h2 className="text-xl font-semibold text-slate-900">Monthly Posted Revenue</h2><p className="text-sm text-slate-500">Revenue recognized from posted journal entries on revenue accounts. Cash collections are reported separately and are not used as revenue.</p></div><div className="p-5">{monthly.length ? <div className="grid gap-3 md:grid-cols-3">{monthly.map((r, i) => <div key={i} className="rounded-xl border border-slate-200 bg-slate-50 p-4"><p className="text-sm text-slate-500">{r?._id?.month || "Unavailable"}/{r?._id?.year || "Unavailable"}</p><p className="mt-1 text-lg font-semibold text-slate-900">{money(r.revenue)}</p><p className="mt-1 text-xs text-slate-500">{isFiniteNumber(r.transactions) ? `${Number(r.transactions).toLocaleString()} posted journal entries` : "Journal-entry count unavailable"}</p></div>)}</div> : <p className="text-sm text-slate-500">No posted journal revenue records are available for this period.</p>}</div></section>
  </div>;
}
