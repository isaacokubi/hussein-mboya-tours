...
export default function Reconciliation() {
  ...
  return <div className="reconciliation-page">
    ...
    <section className="recon-section"><div className="recon-section-heading"><div><p className="recon-kicker">Exceptions register</p><h2>Mismatched payments</h2><span>Only records returned by the reconciliation API are shown.</span></div></div><div className="recon-table-wrap">{!dataSet.mismatches?.length ? <EmptyState text="No payment, invoice or booking mismatches detected." /> : <table className="recon-table"><thead><tr><th>Type</th><th>Booking</th><th>Invoice</th><th>Expected</th><th>Recorded</th><th>Status</th></tr></thead><tbody>{dataSet.mismatches.map((item, index) => <tr key={`${item.bookingId || item.invoiceNumber || "mismatch"}-${item.type}-${index}`}><td><span className="recon-type">{String(item.type || "exception").replaceAll("_", " ")}</span></td><td>{item.bookingNumber || "—"}</td><td>{item.invoiceNumber || "—"}</td><td>{item.expectedPaid !== undefined ? money(item.expectedPaid) : item.expectedStatus || item.expectedNetCommission || "—"}</td><td>{item.storedPaid !== undefined ? money(item.storedPaid) : item.invoicePaid !== undefined ? money(item.invoicePaid) : item.actualStatus || item.actualNetCommission || "—"}</td><td><span className="recon-pill danger">Review</span></td></tr>)}</tbody></table>}</div></section>
