...
export default function AdminFinance() {
  ...
  const finance = unwrap(financeQ.data); const operations = unwrap(operationsQ.data); const payables = list(payablesQ.data); const corporate = list(corporateQ.data); const procurement = operations.procurement || {};
  const outstandingPayables = payables.reduce((sum, item) => sum + Number(item.balance ?? item.outstanding ?? item.amount ?? 0), 0); const corporateExposure = corporate.reduce((sum, item) => sum + Number(item.outstandingBalance ?? item.currentBalance ?? item.balance ?? 0), 0);
  ...
  const loading = [financeQ, operationsQ, payablesQ, corporateQ].some((query) => query.isLoading); const errors = [financeQ, operationsQ, payablesQ, corporateQ].filter((query) => query.isError).length;
  return <div className="space-y-8"><header><p className="text-sm font-semibold uppercase tracking-wider text-emerald-700">Finance & Accounting</p><h1 className="text-3xl font-black text-slate-900">Accounting & Finance Center</h1><p className="mt-2 max-w-3xl text-slate-500">Revenue, payments, reconciliation, supplier liabilities, corporate exposure, tax workflows, payment links, notes and the tenant-scoped general ledger in one workspace.</p></header>
    {errors > 0 && <div className="rounded-xl border border-amber-200 bg-amber-50 p-4 text-sm text-amber-800">{errors} finance data source{errors > 1 ? "s" : ""} could not be loaded. Available figures are still displayed.</div>}{loading && <div className="rounded-xl border border-slate-200 bg-white p-4 text-sm text-slate-500">Refreshing accounting data...</div>}
    ...
    <FinanceLifecycleCenter /><KenyaTaxCenter /><section id="general-ledger"><AccountingLedger /></section><GoLiveReadiness />
    <section className="rounded-2xl bg-white p-6 shadow-sm ring-1 ring-slate-200"><h2 className="text-xl font-bold">Operational accounting controls</h2><div className="mt-4 grid gap-3 md:grid-cols-2"><p className="rounded-xl bg-slate-50 p-4 text-sm text-slate-600">✓ Revenue, net revenue, refunds and payment status</p><p className="rounded-xl bg-slate-50 p-4 text-sm text-slate-600">✓ Supplier payables, settlements and procurement exposure</p><p className="rounded-xl bg-slate-50 p-4 text-sm text-slate-600">✓ Corporate receivable exposure and credit controls</p><p className="rounded-xl bg-slate-50 p-4 text-sm text-slate-600">✓ Tour costing and profitability records</p><p className="rounded-xl bg-slate-50 p-4 text-sm text-slate-600">✓ Commission tracking and reconciliation</p><p className="rounded-xl bg-slate-50 p-4 text-sm text-slate-600">✓ Tax profile, invoice eTIMS status and compliance workflow</p><p className="rounded-xl bg-slate-50 p-4 text-sm text-slate-600">✓ Payment links and credit/debit note lifecycle with GL posting</p></div></section>
  </div>;
}
