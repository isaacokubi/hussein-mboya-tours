...
function formatUptime(value) {
  if (typeof value === "number" && Number.isFinite(value)) {
    ...
    return days ? `${days}d ${hours}h ${minutes}m` : hours ? `${hours}h ${minutes}m ${seconds}s` : minutes ? `${minutes}m ${seconds}s` : `${seconds}s`;
  }
...
export default function SystemHealth() {
  ...
  if (isLoading) {
    return <div className="p-6 text-slate-600">Checking system health...</div>;
  }
  ...
  const serverStatus = health.server || health.status;
  ...
  const hasDegradedService = Object.values(serviceStatuses).some((status) => ["unavailable", "disconnected", "not_configured"].includes(status));
  const overallLabel = hasDegradedService ? "Degraded" : operationalCount === serviceDefinitions.length ? "Operational" : "Partial";
  const overallTone = hasDegradedService ? "text-amber-700 bg-amber-50 border-amber-200" : operationalCount === serviceDefinitions.length ? "text-emerald-700 bg-emerald-50 border-emerald-200" : "text-slate-700 bg-slate-50 border-slate-200";
  ...
  return (
    <div className="space-y-6 p-6">
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <div className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
          <p className="text-sm font-medium text-slate-500">Service availability</p>
          <p className="mt-2 text-3xl font-bold text-slate-900">{operationalCount}/{serviceDefinitions.length}</p>
          <p className="mt-1 text-sm text-slate-500">{overallLabel.toLowerCase()} core services</p>
        </div>
        <div className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
          <p className="text-sm font-medium text-slate-500">Overall status</p>
          <span className={`mt-3 inline-flex rounded-full border px-3 py-1 text-sm font-semibold ${overallTone}`}>{overallLabel}</span>
          <p className="mt-2 text-xs text-slate-500">Based on the latest health checks</p>
        </div>
        <div className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
          <p className="text-sm font-medium text-slate-500">Uptime</p>
          <p className="mt-2 text-2xl font-bold text-slate-900">{formatUptime(health.uptime)}</p>
          <p className="mt-1 text-sm text-slate-500">Application runtime</p>
        </div>
        <div className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
          <p className="text-sm font-medium text-slate-500">Node.js</p>
          <p className="mt-2 text-2xl font-bold text-slate-900">{health.nodeVersion || "Unknown"}</p>
      ...
      <section>
        <div className="mb-4 flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-slate-500">Live connectivity reported by the platform health endpoint.</p>
          </div>
        ...
        <div className="grid gap-4 md:grid-cols-2">
          {serviceDefinitions.map(({ key, label, description }) => (
            <ServiceCard
              ...
              message={key === "mpesa" ? services.mpesa?.message : key === "cloudinary" ? services.cloudinary?.message : undefined}
              environment={key === "mpesa" ? mpesaEnvironment : undefined}
            />
          ))}
        </div>
      </section>
      ...
      <section className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
        ...
        {String(mpesaEnvironment).toLowerCase() === "sandbox" && (
          <div className="mt-4 rounded-lg border border-amber-200 bg-amber-50 p-4 text-sm text-amber-800">
            M-Pesa connectivity is verified against the sandbox gateway. This confirms connectivity and credentials, but it does not mean production M-Pesa payments are enabled.
          </div>
        )}
      </section>
    </div>
