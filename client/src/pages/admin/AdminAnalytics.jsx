...
export default function AdminAnalytics(
  ...
  const cards = [
    ["Confirmed payment revenue",`settings.currency || "KES" ${Number(revenue.totalRevenue||0).toLocaleString()}`,WalletCards],
  ...
  return <div className="min-h-screen bg-slate-50 p-6"><div className="mx-auto max-w-7xl">
    ...
    <div className="grid gap-6 lg:grid-cols-2">
      <ChartCard title="Monthly collected revenue"><ResponsiveContainer width="100%" height={320}><BarChart data={monthlyRevenue}><XAxis dataKey="_id.month"/><YAxis/><Tooltip formatter={(v)=>`settings.currency || "KES" ${Number(v).toLocaleString()}`}/><Bar dataKey="revenue"/></BarChart></ResponsiveContainer></ChartCard>
