...
export default function AdminAnalytics() {
  ...
  const cards = [
    ["Collected revenue",`KES ${Number(revenue.totalRevenue||0).toLocaleString()}`,WalletCards],
  ...
  return <div className="min-h-screen bg-slate-50 p-6"><div className="mx-auto max-w-7xl">
    ...
    <div className="mt-6 rounded-2xl bg-white p-6 shadow-sm ring-1 ring-slate-200"><div className="flex items-center gap-2"><Trophy className="text-amber-600"/><h2 className="text-xl font-bold">Most popular tours</h2></div><p className="mt-1 text-sm text-slate-500">Ranked by booking count. Revenue is shown only where the booking and payment are both confirmed/paid.</p><div className="mt-5 overflow-x-auto"><table className="w-full"><thead className="bg-slate-50"><tr><th className="p-3 text-left">Rank</th><th className="p-3 text-left">Tour</th><th className="p-3 text-left">Bookings</th><th className="p-3 text-left">Confirmed & paid</th><th className="p-3 text-left">Recognized revenue</th></tr></thead><tbody>{popularTours.map((item,i)=><tr key={item._id||i} className="border-t"><td className="p-3 font-bold">#{i+1}</td><td className="p-3 font-semibold">{item.tour?.title || item.title || "Unknown tour"}</td><td className="p-3">{item.totalBookings||0}</td><td className="p-3">{item.confirmedPaidBookings||0}</td><td className="p-3 font-semibold">KES {Number(item.revenue||0).toLocaleString()}</td></tr>)}{!popularTours.length&&<tr><td colSpan="5" className="p-8 text-center text-slate-500">No tour data yet.</td></tr>}</tbody></table></div></div>
  </div></div>;
}
