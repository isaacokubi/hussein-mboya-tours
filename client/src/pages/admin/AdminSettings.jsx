...
export default function AdminSettings() {
  ...
  if (isLoading) return <div className="p-8">Loading tenant settings...</div>;
  if (isError) return <div className="p-8"><p className="text-red-600">Failed to load tenant settings.</p><button type="button" onClick={() => refetch()} className="mt-3 rounded-lg bg-emerald-700 px-4 py-2 font-semibold text-white">Retry</button></div>;
  ...
  return <div className="min-h-screen bg-slate-50 p-4 md:p-6"><div className="mx-auto max-w-7xl">
    <header className="mb-6 rounded-3xl bg-gradient-to-r from-slate-950 via-emerald-950 to-slate-900 p-6 text-white shadow-xl"><p className="text-sm font-semibold uppercase tracking-widest text-emerald-300">Tenant Administration</p><h1 className="mt-2 text-3xl font-bold">{settings.companyName || tenantProfile?.companyName || "Company"} Settings</h1><p className="mt-2 text-slate-300">Tenant-owned company settings are editable below. Subscription, seat capacity, slug and tenant status remain controlled by the platform owner.</p></header>
    <form onSubmit={save} className="space-y-6">
      {tenantProfile && <Section icon={<Building2 size={20}/>} title="Tenant account & subscription"><div className="mb-4 rounded-xl border border-emerald-100 bg-emerald-50 px-4 py-3 text-sm text-emerald-900">Edit the tenant-owned fields here. Changes are saved to this tenant and propagated to the public website.</div><div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
        <Field label="Company name"><input required value={settings.companyName || ""} onChange={e=>update("companyName",e.target.value)} /></Field>
        <ReadOnlyField label="Legal name" value={tenantProfile.legalName} hint="Managed from the tenant account record" />
        <Field label="Company email"><input type="email" value={settings.supportEmail || tenantProfile.companyEmail || ""} onChange={e=>update("supportEmail",e.target.value)} /></Field>
        <Field label="Company phone"><input value={settings.supportPhone || tenantProfile.companyPhone || ""} onChange={e=>update("supportPhone",e.target.value)} /></Field>
        <ReadOnlyField label="Company slug" value={tenantProfile.slug} hint="Platform-managed identifier" />
        <Field label="Website"><input type="url" value={settings.websiteUrl || tenantProfile.websiteUrl || ""} onChange={e=>update("websiteUrl",e.target.value)} /></Field>
        <Field label="Country"><input value={settings.country || tenantProfile.country || ""} onChange={e=>update("country",e.target.value)} /></Field>
        <Field label="Currency"><input value={settings.currency || tenantProfile.currency || "KES"} onChange={e=>update("currency",e.target.value.toUpperCase())} /></Field>
        <Field label="Timezone"><input value={settings.timezone || tenantProfile.timezone || "Africa/Nairobi"} onChange={e=>update("timezone",e.target.value)} /></Field>
        <ReadOnlyField label="Subscription plan" value={planLabel} hint="Platform-managed" />
        <ReadOnlyField label="User seats" value={tenantProfile.userSeats ? `${tenantProfile.userSeats} seats` : "—"} hint="Platform-managed" />
        <ReadOnlyField label="Tenant status" value={tenantProfile.status} hint="Platform-managed" />
      </div><div className="mt-5 grid gap-4 md:grid-cols-2"><InfoCard icon={<Users size={18}/>} title="User capacity" value={`${tenantProfile.userSeats || 0} seats`} text="Seat capacity is controlled by the platform owner."/><InfoCard icon={<CreditCard size={18}/>} title="Subscription" value={planLabel} text={tenantProfile.trialEndsAt ? `Trial ends ${new Date(tenantProfile.trialEndsAt).toLocaleDateString()}.` : "Subscription details are managed by the platform owner."}/></div>{tenantProfile.primaryAdministrator && <div className="mt-5 rounded-2xl border border-emerald-100 bg-emerald-50 p-5"><div className="mb-3 flex items-center gap-2 font-bold text-emerald-900"><UserCircle size={20}/> Primary administrator</div><div className="grid gap-3 md:grid-cols-3 text-sm"><ReadOnlyField label="Name" value={tenantProfile.primaryAdministrator.name}/><ReadOnlyField label="Email" value={tenantProfile.primaryAdministrator.email}/><ReadOnlyField label="Phone" value={tenantProfile.primaryAdministrator.phone}/></div></div>}</Section>}
      <TenantHeroManager />
      ...
      <Section icon={<PlugZap size={20}/>} title="Existing website → automatic booking capture">
        ...
        <div className="mt-5 rounded-2xl bg-slate-950 p-4 text-xs text-slate-200"><p className="font-bold text-white">Website installation</p><pre className="mt-2 overflow-auto whitespace-pre-wrap">{`<script src="https://YOUR-ERP-DOMAIN/api/integrations/v1/widget.js?siteKey=YOUR_HMT_SITE_KEY"></script>`}</pre><p className="mt-2">Mark the booking form with <code>data-hmt-booking-form</code> and put the tour Firebase Firestore ID in <code>data-tour-id</code>. Common fields such as name, email, phone, travel date, guests, hotel and pickup are captured automatically.</p></div>
      </Section>
...
function ReadOnlyField({label,value,hint}) { return <div><span className="mb-2 flex items-center gap-1 text-sm font-semibold text-slate-700"><Lock size={13}/>{label}</span><div className="min-h-11 rounded-lg border border-slate-200 bg-slate-50 px-3 py-2 text-slate-800">{value || "—"}</div>{hint && <p className="mt-1 text-xs text-slate-500">{hint}</p>}</div>; }
