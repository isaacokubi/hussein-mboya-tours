...
export default function UserManagement() {
  ...
  useEffect(() => {
    const timer = setTimeout(() => setSearch(searchValue), 500);
  ...
  const deleteMutation = useMutation({
    ...
    onMutate: async (userId) => {
      await queryClient.cancelQueries({ queryKey: ["admin-users", search, page] });
  ...
  return (
    <section className="space-y-6 p-6">
      <div className="flex flex-col justify-between gap-4 md:flex-row md:items-center">
        ...
        <button onClick={() => setShowCreate(true)} className="inline-flex items-center justify-center gap-2 rounded-xl bg-emerald-700 px-5 py-3 font-semibold text-white shadow hover:bg-emerald-800"><UserPlus size={18}/> Create user account</button>
      </div>
      <div className="rounded-2xl bg-white p-4 shadow-sm ring-1 ring-slate-200"><div className="flex items-center gap-3 rounded-xl border border-slate-200 px-4 py-2"><Search className="text-slate-400" size={19}/><input value={searchValue} onChange={e => { setSearchValue(e.target.value); setPage(1); }} placeholder="Search by name, email, phone, role or status..." className="w-full p-2 outline-none"/>{search && <button onClick={() => { setSearchValue(""); setSearch(""); setPage(1); }} className="text-sm text-slate-500">Clear</button>}</div></div>
      <div className="overflow-hidden rounded-2xl bg-white shadow-sm ring-1 ring-slate-200"><div className="overflow-x-auto"><table className="w-full"><thead className="bg-slate-50"><tr><th className="p-4 text-left">User</th><th className="p-4 text-left">Role</th><th className="p-4 text-left">Phone</th><th className="p-4 text-left">Status</th><th className="p-4 text-right">Actions</th></tr></thead><tbody>{users.map(user => <tr key={user._id} className="border-t hover:bg-slate-50"><td className="p-4"><div className="font-semibold">{user.name}</div><div className="text-sm text-slate-500">{user.email}</div></td><td className="p-4"><span className="rounded-full bg-slate-100 px-3 py-1 text-sm capitalize">{String(user.role || "customer").replace(/_/g, " ")}</span></td><td className="p-4">{user.phone || "-"}</td><td className="p-4"><span className={`rounded-full px-3 py-1 text-xs font-semibold ${user.isActive ? "bg-emerald-100 text-emerald-700" : "bg-red-100 text-red-700"}`}>{user.isActive ? "Active" : "Disabled"}</span></td><td className="p-4 text-right"><div className="flex justify-end gap-2"><button title={user.isActive ? "Disable" : "Enable"} onClick={() => statusMutation.mutate({ id: user._id, status: user.isActive ? "disabled" : "active" })} className="rounded-lg bg-slate-100 p-2 hover:bg-slate-200">{user.isActive ? <UserX size={18}/> : <UserCheck size={18}/>}</button><button title="Delete" disabled={deleteMutation.isPending && deleteMutation.variables === user._id} onClick={() => window.confirm("Delete this user permanently? Historical bookings and payments will be preserved.") && deleteMutation.mutate(user._id)} className="rounded-lg bg-red-50 p-2 text-red-600 hover:bg-red-100 disabled:opacity-50"><Trash2 size={18}/></button></div></td></tr>)}{!users.length && <tr><td colSpan="5" className="p-10 text-center text-slate-500">No users match your search.</td></tr>}</tbody></table></div><div className="flex items-center justify-between border-t p-4"><span className="text-sm text-slate-500">Page {page} of {totalPages}</span><div className="flex gap-2"><button disabled={page <= 1} onClick={() => setPage(p => p - 1)} className="rounded-lg border p-2 disabled:opacity-40"><ChevronLeft size={18}/></button><button disabled={page >= totalPages} onClick={() => setPage(p => p + 1)} className="rounded-lg border p-2 disabled:opacity-40"><ChevronRight size={18}/></button></div></div></div>
