...
const unwrapMetrics = (payload) => {
  ...
  if (current?.data && typeof current.data === "object" && !Array.isArray(current.data)) {
    ...
    if (nested.users !== undefined || nested.customers !== undefined || nested.customerProfiles !== undefined || nested.customerAccounts !== undefined || nested.vehicles !== undefined || nested.bookings !== undefined || nested.tours !== undefined || nested.destinations !== undefined || nested.payments !== undefined) current = nested;
  }
  return current;
};
const unwrapScope = (payload) => payload?.scope || payload?.data?.scope || payload?.data?.data?.scope || {};
const cards = [["users", "All Platform User Accounts"], ["customerProfiles", "Customer Profiles"], ["customerAccounts", "Customer Accounts"], ["staff", "Staff"], ["agents", "Agents"], ["vehicles", "Vehicles"], ["availableVehicles", "Available Vehicles"], ["bookings", "Bookings"], ["admins", "Tenant Administrators"], ["tours", "Tours"], ["destinations", "Destinations"], ["payments", "Payments"], ["completedPayments", "Completed Payments"]];

export default function SuperAdminDashboard() {
  const { settings = {} } = useSettings() || {};
  const platformName = String(settings.companyName || "Global Tours").trim() || "Global Tours";
  const { data, isLoading, isError, error, refetch, isFetching } = useQuery({ queryKey: ["superadmin-dashboard-metrics"], queryFn: getSuperAdminDashboardMetrics, retry: 1, staleTime: 30_000, refetchOnMount: "always", refetchOnWindowFocus: true });
  ...
  return <main className="min-h-screen space-y-8 bg-gray-50 p-6 md:p-8">
    ...
    <section className="rounded-2xl border bg-white p-5 text-sm text-gray-500 shadow-sm">Customer Profiles are profile records, while Customer Accounts are active user accounts with the customer role. Staff, agents, vehicles, tours, destinations, bookings, and payments are restricted to operational tenant data. The global platform owner account is excluded from tenant-level totals.</section>
