...
export default function Contact() {
  ...
  return <div className="min-w-0 overflow-x-clip bg-white">
    ...
    <section className="py-10 sm:py-14 lg:py-16"><div className="mx-auto grid max-w-7xl grid-cols-1 gap-4 px-4 sm:grid-cols-2 sm:px-6 lg:grid-cols-4 lg:gap-6 lg:px-8">{details.map((item, index) => { const Icon = item.icon; const content = <><Icon size={34} className="mx-auto mb-3 text-yellow-500" /><h3 className="mb-2 text-base font-bold text-green-900 sm:text-lg">{item.title}</h3><p className="break-words text-xs text-gray-600 sm:text-sm">{item.value}</p></>; return <motion.div key={index} whileHover={{ y: -6 }} className="min-w-0 rounded-xl border bg-white p-5 text-center shadow-lg sm:p-6">{item.href ? <a href={item.href} target={item.href.startsWith('http') ? '_blank' : undefined} rel={item.href.startsWith('http') ? 'noreferrer' : undefined} className="block rounded-lg focus:outline-none focus:ring-2 focus:ring-green-500">{content}</a> : content}</motion.div>; })}</div></section>
    <section className="bg-gray-50 py-12 sm:py-16 lg:py-20"><div className="mx-auto grid max-w-7xl grid-cols-1 gap-8 px-4 sm:px-6 lg:grid-cols-2 lg:gap-12 lg:px-8"><div className="min-w-0"><h2 className="mb-5 text-2xl font-bold text-green-900 sm:text-3xl lg:text-4xl">Start Planning Your Journey</h2><p className="mb-7 text-sm leading-relaxed text-gray-600 sm:text-base">Whether you want corporate travel management services, airport transfers, car hire and transport services, meet and assist services, conference and events services, a luxury safari, beach holiday, honeymoon experience, mountain adventure or customized tour, our team will create the perfect package for you.</p><div className="space-y-4 text-sm sm:text-base"><a href={`tel:${BUSINESS_PHONE}`} className="flex items-start gap-3 text-gray-700 hover:text-green-800"><MessageCircle className="mt-0.5 shrink-0 text-yellow-500" />Fast response from our travel consultants — call {BUSINESS_PHONE}</a><a href="https://www.google.com/maps/search/?api=1&query=Nairobi%2C%20Kenya" target="_blank" rel="noreferrer" className="flex items-start gap-3 text-gray-700 hover:text-green-800"><MapPin className="mt-0.5 shrink-0 text-yellow-500" />Our office: {OFFICE_ADDRESS}</a></div></div>
      <motion.form onSubmit={submitHandler} noValidate initial={{ opacity: 0, x: 50 }} animate={{ opacity: 1, x: 0 }} className="min-w-0 rounded-2xl bg-white p-4 shadow-xl sm:p-6 lg:p-8"><h3 className="mb-5 text-xl font-bold text-green-900 sm:text-2xl">Send An Inquiry</h3>{error && <div role="alert" className="mb-4 rounded-lg border border-red-200 bg-red-50 p-3 text-xs text-red-700 sm:text-sm">{error}</div>}{sent && <div role="status" className="mb-4 rounded-lg border border-green-200 bg-green-50 p-3 text-xs text-green-700 sm:text-sm">Your inquiry has been prepared in your email application. Please send the email to complete your inquiry.</div>}<div className="grid grid-cols-1 gap-3 sm:grid-cols-2"><input name="name" value={form.name} onChange={handleChange} placeholder="Full Name" autoComplete="name" required className="w-full min-w-0 rounded-lg border p-2.5 text-sm outline-none transition focus:border-green-700 focus:ring-2 focus:ring-green-100" /><input name="email" value={form.email} onChange={handleChange} placeholder="Email Address" type="email" autoComplete="email" required className="w-full min-w-0 rounded-lg border p-2.5 text-sm outline-none transition focus:border-green-700 focus:ring-2 focus:ring-green-100" /></div><input name="phone" value={form.phone} onChange={handleChange} placeholder="Phone Number" type="tel" autoComplete="tel" className="mt-3 w-full min-w-0 rounded-lg border p-2.5 text-sm outline-none transition focus:border-green-700 focus:ring-2 focus:ring-green-100" /><textarea name="message" value={form.message} onChange={handleChange} placeholder="Tell us about your trip..." rows={6} required className="mt-3 w-full min-w-0 resize-y rounded-lg border p-2.5 text-sm outline-none transition focus:border-green-700 focus:ring-2 focus:ring-green-100" /><button type="submit" className="mt-4 flex w-full items-center justify-center gap-2 rounded-xl bg-green-900 py-3 text-sm font-bold text-white transition hover:bg-green-800"><Send size={18} />Send Message</button><p className="mt-2 text-center text-[11px] text-gray-500">Inquiries are sent to {BUSINESS_EMAIL}.</p></motion.form>
    </div></section>
