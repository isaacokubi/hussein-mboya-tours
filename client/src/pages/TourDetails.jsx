    ...
    <div className="min-h-screen bg-gray-100 p-6">
      <div className="max-w-7xl mx-auto bg-white rounded-3xl shadow-xl p-8 grid md:grid-cols-2 gap-10">
        ...
        <section className="md:col-span-2 mt-2 rounded-2xl bg-gray-50 p-6">
          <div className="flex items-center gap-2"><Star className="text-yellow-500" fill="currentColor" /><h2 className="text-2xl font-bold">Customer Reviews</h2></div>
          <p className="mt-1 text-gray-500">{reviews.length ? `${reviews.length} verified review${reviews.length === 1 ? "" : "s"}` : "No approved reviews yet."}</p>
          <div className="mt-5 grid gap-4 md:grid-cols-2">
            {reviews.map((review) => (
              <article key={review._id} className="rounded-xl bg-white p-5 shadow-sm">
                <div className="flex items-center justify-between gap-3"><strong>{review.user?.name || "Traveler"}</strong><span className="text-yellow-500">{"★".repeat(Number(review.rating || 0))}</span></div>
