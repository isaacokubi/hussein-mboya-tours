...
export default function AgentDashboard() {
  ...
  if (isError) return <div className="m-6 rounded-xl border border-red-200 bg-red-50 p-5 text-red-700"><p className="font-semibold">Agent dashboard unavailable</p><p className="mt-1 text-sm">{statusCode === 401 ? "Your session has expired. Please sign in again." : statusCode === 403 ? "Your account is not authorized for the agent dashboard." : error?.response?.data?.message || error?.message || "Unable to load the agent dashboard."}</p><div className="mt-4 flex gap-2"><button onClick={() => refetch()} disabled={isFetching} className="rounded-lg bg-red-700 px-4 py-2 text-sm font-medium text-white disabled:opacity-60">{isFetching ? "Retrying..." : "Retry"}</button>{statusCode === 401 && <a href="/login" className="rounded-lg border border-red-300 bg-white px-4 py-2 text-sm font-medium">Sign in</a>}</div></div>;

  return <section className="space-y-6 p-6">
    <div className="flex flex-col justify-between gap-3 sm:flex-row sm:items-center"><div><h1 className="text-3xl font-bold text-gray-900">Agent Dashboard</h1><p className="mt-1 text-sm text-gray-500">{payload?.agent?.companyName || "Agent operations"}</p></div><div className="flex items-center gap-3"><span className={`rounded-full px-3 py-1 text-xs font-semibold ${isApproved ? "bg-green-100 text-green-700" : "bg-amber-100 text-amber-700"}`}>{isApproved ? "Approved" : "Pending approval"}</span><button onClick={() => refetch()} disabled={isFetching} className="rounded-lg border bg-white px-4 py-2 text-sm font-medium shadow-sm hover:bg-gray-50 disabled:opacity-60">{isFetching ? "Refreshing..." : "Refresh"}</button></div></div>
