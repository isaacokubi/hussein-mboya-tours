...
export default function TourManagerCommissions() {
  ...
  const withdrawalsQuery = useQuery({ queryKey: ["agent-withdrawals"], queryFn: getAgentWithdrawals, staleTime: 10_000 });
  ...
  const approveWithdrawal = useMutation({ mutationFn: approveAgentWithdrawal, onSuccess: () => { refreshWithdrawals(); toast.success("Withdrawal approved."); }, onError: (error) => toast.error(error?.response?.data?.message || "Unable to approve withdrawal.") });
  ...
  const completeWithdrawal = useMutation({ mutationFn: ({ id, reference }) => completeAgentWithdrawal(id, reference), onSuccess: () => { setWithdrawal(null); refreshWithdrawals(); toast.success("Withdrawal completed and deducted from the agent wallet."); }, onError: (error) => toast.error(error?.response?.data?.message || "Unable to complete withdrawal.") });
  ...
  return (
    <section className="min-h-full bg-slate-50 p-6 md:p-8">
      <div className="mx-auto max-w-7xl">
        <div className="mb-6"><p className="text-sm font-semibold uppercase tracking-wider text-emerald-700">Finance</p><h1 className="text-3xl font-bold text-slate-900">Agent Commission & Withdrawals</h1><p className="mt-1 text-slate-500">Approve commissions, pay them into the agent wallet, then approve and complete agent withdrawal requests.</p></div>
        ...
        <div className="mt-8 overflow-hidden rounded-2xl bg-white shadow-sm ring-1 ring-slate-200">
          <div className="border-b p-5"><h2 className="text-xl font-bold text-slate-900">Agent withdrawal requests</h2><p className="mt-1 text-sm text-slate-500">A completed withdrawal is deducted from the agent's available commission wallet.</p></div>
      ...
      {withdrawal && <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/60 p-4"><form onSubmit={(e) => { e.preventDefault(); const reference = String(new FormData(e.currentTarget).get("reference") || "").trim(); if (!reference) return toast.error("Enter the M-Pesa or bank transaction reference."); completeWithdrawal.mutate({ id: withdrawal._id, reference }); }} className="w-full max-w-lg rounded-2xl bg-white p-6 shadow-2xl"><h2 className="text-xl font-bold text-slate-900">Complete agent withdrawal</h2><p className="mt-1 text-sm text-slate-500">{withdrawal.agent?.user?.name || withdrawal.agent?.companyName || "Agent"} · {money(withdrawal.amount)} · {withdrawal.method === "MPESA" ? withdrawal.mpesaPhone : `${withdrawal.bankName} · ${withdrawal.bankAccountNumber}`}</p><div className="mt-5"><label className="block"><span className="mb-1 block text-sm font-semibold">Actual payout reference / transaction ID</span><input name="reference" required placeholder="Enter the real transfer reference" className="w-full rounded-lg border px-3 py-2" /></label><p className="mt-2 text-xs text-slate-500">Only mark this completed after the money has actually been sent through your M-Pesa or bank process.</p></div><div className="mt-6 flex justify-end gap-3"><button type="button" onClick={() => setWithdrawal(null)} className="rounded-lg border px-4 py-2 font-semibold">Cancel</button><button type="submit" disabled={completeWithdrawal.isPending} className="rounded-lg bg-emerald-700 px-4 py-2 font-semibold text-white">{completeWithdrawal.isPending ? "Completing..." : "Mark payout completed"}</button></div></form></div>}
    </section>
  );
}
