...
import { LayoutDashboard, Map, CalendarCheck, Wallet, Users, Car, Settings, Smartphone, FileText, Home, Shield, BarChart3, UserRoundCog, CreditCard } from "lucide-react";
...
const sections = [
  {
    ...
    items: [
      ...
      ["Agents", "/admin/agents", Users, "agent.manage"],
...
const ADMIN_CORE_PERMISSIONS = new Set([
  "admin.dashboard",
  "booking.manage",
  "tour.manage",
  "customer.view",
...
export default function AdminSidebar() {
  ...
  const canRender = (permission) =>
    !permission ||
    (adminUser && ADMIN_CORE_PERMISSIONS.has(permission)) ||
    hasPermission(permission);

  return (
    <div>
      <div className="ops-brand">
        <div className="ops-brand-mark">CT</div>
        <div>
      ...
      {sections.map((section) => (
        <div key={section.title}>
          <div className="ops-section">{section.title}</div>
      ...
      <div className="ops-alert" style={{ marginTop: 20 }}>
        Operational mode: monitor bookings, payments, schedules and resources from one control surface.
      </div>
    </div>
  );
}
