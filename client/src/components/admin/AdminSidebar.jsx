...
import { LayoutDashboard, Map, CalendarCheck, Wallet, Users, Car, Settings, Smartphone, FileText, Home, Shield, BarChart3, UserRoundCog, PackageCheck, Receipt } from "lucide-react";
...
export default function AdminSidebar() {
  ...
  return <div><div className="ops-brand"><div className="ops-brand-mark">CT</div><div><div className="ops-brand-title">{companyName || "Company"}</div><div className="ops-brand-sub">Operations Center</div></div></div>{sections.map(section => <div key={section.title}><div className="ops-section">{section.title}</div><nav className="ops-nav">{section.items.map(([name, path, Icon, permission]) => canRender(permission) ? <NavLink key={path} to={path} end={path === "/admin"} className={({ isActive }) => `ops-link ${isActive ? "active" : ""}`}><Icon size={17}/><span>{name}</span></NavLink> : null)}</nav></div>)}<div className="ops-alert" style={{marginTop:20}}>Operational mode: monitor bookings, payments, schedules, resources, procurement, accounting and compliance from one control surface.</div></div>;
}
