...
const BOOKING_STATUSES = ["confirmed", "pending", "completed", "cancelled", "refunded"];
...
export default function AdminDashboard() {
  ...
  const bookingStatusCounts = useMemo(() => {
    ...
    bookingStatus.forEach((item) => {
      const rawStatus = item?.status ?? (typeof item?._id === "object" ? item?._id?.status : item?._id);
      const status = String(rawStatus || "").trim().toLowerCase();
      if (Object.prototype.hasOwnProperty.call(counts, status)) counts[status] += Number(item?.count || 0);
    });
    return counts;
  }, [bookingStatus]);
  const pendingAgents = useMemo(() => agents.filter((agent) => !agent.isApproved && String(agent.status || "").toLowerCase() !== "approved"), [agents]);
