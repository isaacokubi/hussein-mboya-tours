...
export default function AdminDashboard() {
  ...
  if (isError) return <div className="min-h-screen bg-gray-50 p-8"><div className="max-w-2xl rounded-xl border border-red-200 bg-white p-6 shadow-sm"><h2 className="font-semibold text-red-700">Unable to load Admin Dashboard</h2><p className="mt-2 text-gray-600">{error?.response?.data?.message || error?.message || "Dashboard request failed."}</p><button type="button" onClick={() => void refetch()} className="mt-4 rounded-lg bg-gray-900 px-4 py-2 text-white">Retry</button></div></div>;

  return <div className="min-h-screen space-y-8 bg-gray-50 p-6 lg:p-8">
    <DashboardHeader />
    <div className="flex flex-wrap items-center justify-between gap-3"><div><h2 className="text-2xl font-bold text-slate-900">Business Overview</h2><p className="text-sm text-slate-500">Live tenant-scoped operational, booking and payment metrics.</p></div><button type="button" onClick={() => void refetch()} disabled={isFetching} className="rounded-lg border bg-white px-4 py-2 text-sm font-medium shadow-sm">{isFetching ? "Refreshing..." : "Refresh Dashboard"}</button></div>
    <StatsGrid stats={dashboard} summary={summary} />
    <section className="rounded-2xl bg-white p-6 shadow-sm ring-1 ring-amber-200"><div className="flex flex-wrap items-start justify-between gap-4"><div><div className="flex items-center gap-2"><Clock3 className="h-5 w-5 text-amber-600" /><h2 className="text-xl font-bold">Agent Approvals</h2></div><p className="mt-1 text-sm text-slate-500">Review and approve agent accounts before they begin agent operations.</p></div><div className="rounded-full bg-amber-50 px-3 py-1.5 text-sm font-semibold text-amber-700"><Users className="mr-1 inline h-4 w-4" />{agentsLoading ? "Loading..." : `${pendingAgents.length} pending`}</div></div>{agentsError ? <div className="mt-5 rounded-xl border border-red-200 bg-red-50 p-4 text-sm text-red-700">Unable to load agent approval requests.</div> : agentsLoading ? <div className="mt-5 rounded-xl bg-slate-50 p-5 text-sm text-slate-500">Loading agent approval requests...</div> : pendingAgents.length === 0 ? <div className="mt-5 rounded-xl border border-emerald-200 bg-emerald-50 p-5"><div className="flex items-center gap-2 font-semibold text-emerald-800"><CheckCircle2 className="h-5 w-5" />All agent accounts are approved</div></div> : <div className="mt-5 overflow-x-auto rounded-xl border border-slate-200"><table className="w-full min-w-[720px] text-sm"><thead className="bg-slate-50 text-left text-xs uppercase tracking-wide text-slate-500"><tr><th className="px-4 py-3">Agent</th><th className="px-4 py-3">Company</th><th className="px-4 py-3">Email</th><th className="px-4 py-3">Status</th><th className="px-4 py-3 text-right">Action</th></tr></thead><tbody>{pendingAgents.slice(0, 10).map((a) => <tr key={a._id} className="border-t"><td className="px-4 py-4 font-semibold">{a.user?.name || a.name || "Unnamed agent"}</td><td className="px-4 py-4">{a.companyName || "—"}</td><td className="px-4 py-4">{a.user?.email || a.email || "—"}</td><td className="px-4 py-4"><span className="rounded-full bg-amber-50 px-2.5 py-1 text-xs font-semibold text-amber-700">Pending approval</span></td><td className="px-4 py-4 text-right"><button type="button" onClick={() => approve.mutate(a._id)} disabled={approve.isPending} className="rounded-lg bg-emerald-600 px-3 py-2 font-semibold text-white disabled:opacity-60">{approve.isPending && approve.variables === a._id ? "Approving..." : "Approve Agent"}</button></td></tr>)}</tbody></table></div>}</section>
    ...
    {(monthlyRevenue.length > 0 || bookingStatus.length > 0) && <div className="grid gap-6 lg:grid-cols-2"><section className="rounded-xl bg-white p-6 shadow"><h2 className="mb-5 text-xl font-bold">Revenue Trend</h2>{monthlyRevenue.length === 0 ? <p className="text-gray-500">No completed payment revenue yet.</p> : monthlyRevenue.slice(-6).map((x) => <div key={x.month} className="flex items-center justify-between border-b py-2"><span className="text-gray-600">{x.month}</span><strong>{currency} {Number(x.amount || 0).toLocaleString()}</strong></div>)}</section><section className="rounded-xl bg-white p-6 shadow"><div className="mb-5 flex items-center justify-between"><div><h2 className="text-xl font-bold">Booking Status</h2><p className="mt-1 text-sm text-gray-500">Current booking distribution for this tenant.</p></div><span className={`rounded-full px-3 py-1 text-xs font-semibold ${statusTotal === bookingTotal ? "bg-emerald-50 text-emerald-700" : "bg-red-50 text-red-700"}`}>{statusTotal}/{bookingTotal} accounted for</span></div><div className="grid grid-cols-2 gap-3 sm:grid-cols-3">{BOOKING_STATUSES.map((s) => <div key={s} className={`rounded-xl border p-4 ${statusStyles[s]}`}><div className="flex items-center justify-between gap-3"><span className="text-sm font-semibold capitalize">{s}</span><strong className="text-2xl">{bookingStatusCounts[s].toLocaleString()}</strong></div></div>)}</div>{statusTotal !== bookingTotal && <p className="mt-4 rounded-lg bg-red-50 p-3 text-sm text-red-700">Dashboard data check: booking status totals do not equal total bookings. Review any unknown booking statuses instead of silently hiding them.</p>}</section></div>}
    <QuickActions />
  </div>;
}
