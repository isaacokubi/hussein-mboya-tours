...
export default function AdminDashboard() {
  ...
  const approve = useMutation({
    ...
    onError: (mutationError) => {
      toast.error(mutationError?.response?.data?.message || "Unable to approve agent.");
    },
  });
    ...
    return (
      <div className="min-h-screen bg-gray-50 p-8">
        <div className="max-w-2xl rounded-xl border border-red-200 bg-white p-6 shadow-sm">
          ...
          <p className="mt-2 text-gray-600">{status === 401 ? "Your session has expired. Please sign in again." : status === 403 ? "Your account is not authorized to use the Admin Dashboard." : message}</p>
          <button type="button" onClick={() => void refetch()} disabled={isFetching} className="mt-4 rounded-lg bg-gray-900 px-4 py-2 text-white disabled:opacity-60">{isFetching ? "Retrying..." : "Retry"}</button>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen space-y-8 bg-gray-50 p-6 lg:p-8">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div><h2 className="text-2xl font-bold text-slate-900">Business Overview</h2><p className="text-sm text-slate-500">Live tenant-scoped operational, booking and payment metrics.</p></div>
        <button type="button" onClick={() => void refetch()} disabled={isFetching} className="rounded-lg border bg-white px-4 py-2 text-sm font-medium shadow-sm disabled:opacity-60">{isFetching ? "Refreshing..." : "Refresh Dashboard"}</button>
      </div>
      ...
      {(monthlyRevenue.length > 0 || bookingStatus.length > 0) && <div className="grid gap-6 lg:grid-cols-2">
        <section className="rounded-xl bg-white p-6 shadow"><h2 className="mb-5 text-xl font-bold">Revenue Trend</h2>{monthlyRevenue.length === 0 ? <p className="text-gray-500">No completed payment revenue yet.</p> : <div className="space-y-3">{monthlyRevenue.slice(-6).map((item) => <div key={item.month} className="flex items-center justify-between border-b pb-2"><span className="text-gray-600">{item.month}</span><strong>{currencyLabel} {Number(item.amount || 0).toLocaleString()}</strong></div>)}</div>}</section>
