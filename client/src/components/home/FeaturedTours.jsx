...
export default function FeaturedTours() {
      ...
      <section className="py-20">
        <div className="container mx-auto px-6">
          <div className="grid md:grid-cols-3 gap-8">
            {[1, 2, 3].map((item) => (
              <div key={item} className="h-96 bg-gray-200 animate-pulse rounded-xl" />
            ))}
          </div>
        </div>
      </section>
  ...
  if (isError) {
    return (
      <section className="py-20 text-center">
        <p className="text-red-600 font-semibold">Failed to load featured tours.</p>
  ...
  return (
    <section className="py-20 bg-gradient-to-b from-white via-gray-50 to-green-50">
      <div className="container mx-auto px-6">
        ...
        {tours.length === 0 ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {tours.map((tour, index) => {
              // A stale/broken stored URL must never send the card back to the
              // missing /images/image-placeholder.jpg asset.
              const fallbackImage =
