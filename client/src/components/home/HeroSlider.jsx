...
export default function HeroSlider() {
  ...
  return <section className="relative overflow-hidden px-1 py-1">
    <div className="relative h-[58vh] min-h-[430px] max-h-[820px] overflow-hidden rounded-xl shadow-2xl sm:h-[68vh] lg:h-[78vh]" style={{ backgroundColor: "var(--tenant-surface, #ffffff)" }}>
    <Swiper modules={[Autoplay, EffectFade]} effect="fade" speed={900} autoplay={{ delay: 6000, disableOnInteraction: false }} loop={slides.length > 1} preloadImages={false} className="h-full" onSwiper={(swiper) => { pauseAllVideos(); window.setTimeout(() => playVideo(swiper.realIndex), 100); }} onSlideChange={(swiper) => { pauseAllVideos(); window.setTimeout(() => playVideo(swiper.realIndex), 100); }}>
      {slides.map((slide, index) => {
        ...
        return <SwiperSlide key={slide._id || index}><div className="relative h-full overflow-hidden bg-slate-950">
          ...
          <div className="absolute inset-0" style={{ background: `linear-gradient(to bottom, rgb(0 0 0 / .35), rgb(0 0 0 / .45), rgb(0 0 0 / ${Number(settings.heroOverlayOpacity ?? 50) / 100}))` }} />
          <div className="relative z-10 flex h-full flex-col items-center justify-center px-4 text-center text-white sm:px-6">
            ...
            {(slide.buttonText || slide.ctaText) && (slide.buttonLink || slide.ctaLink) && <Link to={slide.buttonLink || slide.ctaLink} className="mt-6 inline-flex max-w-full items-center justify-center rounded-full px-5 py-2.5 text-sm font-semibold text-white shadow-lg transition hover:scale-105 sm:mt-8 sm:px-7 sm:py-3" style={{ backgroundColor: "var(--tenant-primary, #047857)" }}>{slide.buttonText || slide.ctaText}</Link>}
          </div>
        </div></SwiperSlide>;
      })}
    </Swiper>
