...
export default function PlanFeatureGate({ children }) {
  ...
  if (state.loading || state.features === null) return <div className="min-h-[55vh] flex items-center justify-center bg-slate-50 p-6"><div className="w-full max-w-md rounded-3xl border border-slate-200 bg-white p-8 text-center shadow-sm"><div className="mx-auto flex h-12 w-12 items-center justify-center rounded-2xl bg-emerald-50 text-emerald-700"><RefreshCw size={22} className="animate-spin" /></div><h1 className="mt-4 text-lg font-bold text-slate-900">Checking workspace access</h1><p className="mt-2 text-sm text-slate-500">Verifying your subscription entitlement for this module.</p></div></div>;
  if (state.error || state.features.includes(feature)) return children;

  const featureLabel = labelFor(feature);
  return <div className="min-h-[70vh] bg-slate-50 p-5 md:p-8"><div className="mx-auto flex min-h-[60vh] max-w-2xl items-center justify-center"><section className="w-full overflow-hidden rounded-[2rem] border border-amber-200 bg-white shadow-xl"><div className="bg-gradient-to-br from-emerald-950 via-emerald-900 to-slate-900 p-8 text-white md:p-10"><div className="flex h-14 w-14 items-center justify-center rounded-2xl bg-white/10 ring-1 ring-white/15"><LockKeyhole size={27} /></div><p className="mt-6 text-xs font-black uppercase tracking-[0.18em] text-amber-300">Subscription feature lock</p><h1 className="mt-2 text-3xl font-black tracking-tight md:text-4xl">{featureLabel}</h1><p className="mt-3 max-w-xl text-sm leading-6 text-slate-300">This module is not included in the <strong className="text-white">{labelFor(state.plan || "starter")}</strong> workspace plan. Upgrade the subscription to unlock this feature.</p></div><div className="p-8 md:p-10"><div className="grid gap-3 sm:grid-cols-3"><Info icon={ShieldCheck} label="Current plan" value={labelFor(state.plan || "starter")} /><Info icon={LockKeyhole} label="Required access" value={featureLabel} /><Info icon={Sparkles} label="Access status" value="Locked" /></div><div className="mt-7 flex flex-col gap-3 sm:flex-row"><Link to="/admin/billing" className="inline-flex flex-1 items-center justify-center rounded-xl bg-emerald-700 px-5 py-3 text-sm font-bold text-white transition hover:bg-emerald-800">Review subscription</Link><button type="button" onClick={() => window.location.reload()} className="inline-flex flex-1 items-center justify-center gap-2 rounded-xl border border-slate-300 px-5 py-3 text-sm font-bold text-slate-700 transition hover:bg-slate-50"><RefreshCw size={16} /> Recheck access</button></div></div></section></div></div>;
}

function Info({ icon: Icon, label, value }) { return <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4"><div className="flex items-center gap-2 text-xs font-bold uppercase tracking-wide text-slate-500"><Icon size={14} /> {label}</div><p className="mt-2 text-sm font-black capitalize text-slate-900">{value}</p></div>; }
