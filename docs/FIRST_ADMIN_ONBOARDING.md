...
The current repository test exercises this flow only against a disposable loopback MongoDB replica-set database named `first_tenant_acceptance*`. Its local run was skipped because only MongoDB 3.6.8 is installed and Docker is unavailable; CI is configured to run the test on MongoDB 8, but no CI result for this local candidate was obtained. Do not treat this test description as evidence that a production database or first tenant has been accepted.
