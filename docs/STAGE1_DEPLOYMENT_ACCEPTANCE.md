...
For Render, set the backend's `MONGODB_URI` to an Atlas URI explicitly targeting the existing `husseindb` database (the URI path must end in `/husseindb` before any query options), strong `JWT_SECRET`, dedicated payment/webhook encryption keys, HTTPS `CLIENT_URL` and `CLIENT_ORIGINS`, and actual `PLATFORM_HOST`. Production startup rejects a URI that omits or selects a different database. Set the static frontend's `VITE_API_URL` to the public API origin ending in `/api`; set `VITE_SOCKET_URL` to the API origin or allow the client to derive it from `VITE_API_URL`. These values must not be committed to the repository. The deployed current source remains **NOT VERIFIED** until the live GitHub Actions smoke check passes.
